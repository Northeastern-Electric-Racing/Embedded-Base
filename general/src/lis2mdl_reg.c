...
int32_t lis2mdl_operating_mode_set(const stmdev_ctx_t *ctx, lis2mdl_md_t val)
{
	...
	if (ret == 0) {
		reg.md = (uint8_t)val;
		ret = lis2mdl_write_reg(ctx, LIS2MDL_CFG_REG_A, (uint8_t *)&reg,
					1);
	}
				       ...
				       lis2mdl_lpf_t val)
{
	...
	if (ret == 0) {
		reg.lpf = (uint8_t)val;
		ret = lis2mdl_write_reg(ctx, LIS2MDL_CFG_REG_B, (uint8_t *)&reg,
					1);
	}
...
int32_t lis2mdl_block_data_update_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		reg.bdu = val;
		ret = lis2mdl_write_reg(ctx, LIS2MDL_CFG_REG_C, (uint8_t *)&reg,
					1);
	}
