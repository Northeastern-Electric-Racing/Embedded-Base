...
{
	return HAL_I2C_Mem_Read(max->i2c_handle, max->dev_addr, address,
				I2C_MEMADD_SIZE_8BIT, data, REG_SIZE, TIMEOUT);
}

HAL_StatusTypeDef write_reg(max7314_t *max, uint16_t address, uint8_t *data)
{
	return HAL_I2C_Mem_Write(max->i2c_handle, max->dev_addr, address,
				 I2C_MEMADD_SIZE_8BIT, data, REG_SIZE, TIMEOUT);
}
...
{
	return HAL_I2C_Mem_Write(max->i2c_handle, max->dev_addr, address,
				 I2C_MEMADD_SIZE_8BIT, data, REG_SIZE * 2,
				 TIMEOUT);
}
				       ...
				       uint8_t mode)
{
     ...
     */
		...
		status =
			write_reg(max, MAX7314_PHASE_0_OUTPUTS_0_7, &conf_data);
		if (status != HAL_OK)
			return status;
		...
		status =
			read_reg(max, MAX7314_PHASE_0_OUTPUTS_8_15, &conf_data);
		if (status != HAL_OK)
			return status;

		conf_data = (conf_data & ~(1u << (pin - REG_SIZE_BITS))) |
			    (mode << (pin - REG_SIZE_BITS));
		status = write_reg(max, MAX7314_PHASE_0_OUTPUTS_8_15,
				   &conf_data);
		if (status != HAL_OK)
			return status;
	}
...
HAL_StatusTypeDef max7314_set_pin_state(max7314_t *max, uint8_t pin, bool state)
{
	...
	if (pin < 8) {
		...
		pin_data = (pin_data & ~(1u << (pin - REG_SIZE_BITS))) |
			   (pin_state << (pin - REG_SIZE_BITS));
	}
