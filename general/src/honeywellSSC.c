...
int honeywellSSC_read_pressure(honeywellSSC_t *ssc, float *result)
{
	...
	uint16_t status_and_pressure_data = ((uint16_t)data_array[0] << 8) |
					    data_array[1];
	...
	float pressure_reading = (((pressure_data - ssc->min_pressure) *
				   (ssc->max_pressure - ssc->min_pressure)) /
					  (SSC_OUTPUT_MAX - SSC_OUTPUT_MIN) +
				  ssc->min_pressure);
...
int honeywellSSC_read_temp(honeywellSSC_t *ssc, float *result)
{
	...
	float temperature_reading =
		((float)temp_data / SSC_TEMP_COUNTS_MAX) * SSC_TEMP_SCALE +
		SSC_TEMP_OFFSET;
					...
					float *temp_result)
{
	...
	uint16_t status_and_pressure_data = ((uint16_t)data_array[0] << 8) |
					    data_array[1];
	uint16_t pressure_data = status_and_pressure_data & SSC_PRESSURE_MASK;
	uint16_t temp_data = ((((uint16_t)data_array[2] << 8) | data_array[3]) &
			      SSC_TEMP_MASK) >>
			     5;
