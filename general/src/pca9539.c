...
{
	return pca->I2C_Read(pca->dev_addr, reg_type, *buf);
}

int pca9539_write_reg(pca9539_t *pca, uint16_t reg_type, uint8_t buf)
{
	return pca->I2C_Write(pca->dev_addr, reg_type, buf);
}
		      ...
		      uint8_t buf)
{
	...
	int status = pca->I2C_Write(pca->dev_addr, reg_type, buf);
	if (status) {
		return status;
	}
	//uint8_t data_new;
		     ...
		     uint8_t *buf)
{
	...
	int status = pca->I2C_Read(pca->dev_addr, reg_type, *buf);
	if (status) {
		return status;
	}
