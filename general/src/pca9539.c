...
{
	return pca->read(pca, reg_type, buf);
}

int pca9539_write_reg(pca9539_t *pca, uint8_t reg_type, uint8_t buf)
{
	return pca->write(pca, reg_type, &buf);
}
		      ...
		      uint8_t buf)
{
	...
	//	return status;
	//}
	int status = pca->write(pca, reg_type, &data);
	if (status) {
		return status;
	}
	...
	return pca->write(pca, reg_type, &data);
}
		     ...
		     uint8_t *buf)
{
	...
	int status = pca->read(pca, reg_type, &data);
	if (status) {
		return status;
	}
	//*buf = (data & (1 << pin)) > 0; What to do with this?

	return pca->read(pca, reg_type, &data);
}
