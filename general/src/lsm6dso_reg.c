...
/**
  ...
  *
  */
int32_t __weak lsm6dso_read_reg(stmdev_ctx_t *ctx, uint8_t reg, uint8_t *data,
				uint16_t len)
{
	int32_t ret;
...
/**
  ...
  *
  */
int32_t __weak lsm6dso_write_reg(stmdev_ctx_t *ctx, uint8_t reg, uint8_t *data,
				 uint16_t len)
{
	int32_t ret;
...
int32_t lsm6dso_xl_full_scale_set(stmdev_ctx_t *ctx, lsm6dso_fs_xl_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_CTRL1_XL, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_xl_data_rate_set(stmdev_ctx_t *ctx, lsm6dso_odr_xl_t val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_CTRL1_XL, (uint8_t *)&reg,
				       1);
	}
...
int32_t lsm6dso_gy_full_scale_set(stmdev_ctx_t *ctx, lsm6dso_fs_g_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_CTRL2_G, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_gy_data_rate_set(stmdev_ctx_t *ctx, lsm6dso_odr_g_t val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_CTRL2_G, (uint8_t *)&reg,
				       1);
	}
...
int32_t lsm6dso_block_data_update_set(stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		reg.bdu = val;
		ret = lsm6dso_write_reg(ctx, LSM6DSO_CTRL3_C, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_xl_offset_weight_set(stmdev_ctx_t *ctx, lsm6dso_usr_off_w_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_CTRL6_C, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_xl_power_mode_set(stmdev_ctx_t *ctx, lsm6dso_xl_hm_mode_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_CTRL5_C,
					(uint8_t *)&ctrl5_c, 1);
	}

	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_CTRL6_C,
				       (uint8_t *)&ctrl6_c, 1);
	}

	if (ret == 0) {
		ctrl6_c.xl_hm_mode = (uint8_t)val & 0x01U;
		ret = lsm6dso_write_reg(ctx, LSM6DSO_CTRL6_C,
					(uint8_t *)&ctrl6_c, 1);
	}
...
int32_t lsm6dso_gy_power_mode_set(stmdev_ctx_t *ctx, lsm6dso_g_hm_mode_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_CTRL7_G, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_timestamp_set(stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_CTRL10_C, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_rounding_mode_set(stmdev_ctx_t *ctx, lsm6dso_rounding_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_CTRL5_C, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_steps_reset(stmdev_ctx_t *ctx)
{
	...
	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_EMB_FUNC_SRC,
				       (uint8_t *)&reg, 1);
	}

	if (ret == 0) {
		reg.pedo_rst_step = PROPERTY_ENABLE;
		ret = lsm6dso_write_reg(ctx, LSM6DSO_EMB_FUNC_SRC,
					(uint8_t *)&reg, 1);
	}
...
int32_t lsm6dso_odr_cal_reg_set(stmdev_ctx_t *ctx, uint8_t val)
{
	...
	ret = lsm6dso_read_reg(ctx, LSM6DSO_INTERNAL_FREQ_FINE, (uint8_t *)&reg,
			       1);
				 ...
				 uint8_t *val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_PAGE_RW,
				       (uint8_t *)&page_rw, 1);
	}

	if (ret == 0) {
		page_rw.page_rw = 0x02; /* page_write enable */
		ret = lsm6dso_write_reg(ctx, LSM6DSO_PAGE_RW,
					(uint8_t *)&page_rw, 1);
	}
			    ...
			    uint8_t len)
{
	...
	if (ret == 0) {
		for (i = 0; ((i < len) && (ret == 0)); i++) {
			ret = lsm6dso_write_reg(ctx, LSM6DSO_PAGE_VALUE,
						&buf[i], 1);
			addr_pointed++;
			...
			if (((addr_pointed % 0x0100U) == 0x00U) && (ret == 0)) {
				...
				if (ret == 0) {
					page_sel.page_sel =
						((uint8_t)(addr_pointed >> 8) &
						 0x0FU);
					page_sel.not_used_01 = 1;
					ret = lsm6dso_write_reg(
						ctx, LSM6DSO_PAGE_SEL,
						(uint8_t *)&page_sel, 1);
				}
			}
		}
...
int32_t lsm6dso_gy_filter_lp1_set(stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		reg.lpf1_sel_g = val;
		ret = lsm6dso_write_reg(ctx, LSM6DSO_CTRL4_C, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_xl_lp2_on_6d_set(stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_CTRL8_XL, (uint8_t *)&reg,
					1);
	}
					...
					lsm6dso_slope_fds_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_TAP_CFG0, (uint8_t *)&reg,
					1);
	}
				     ...
				     lsm6dso_ois_pu_dis_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_PIN_CTRL, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_aux_xl_self_test_set(stmdev_ctx_t *ctx, lsm6dso_st_xl_ois_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_INT_OIS, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_aux_den_mode_set(stmdev_ctx_t *ctx, lsm6dso_lvl2_ois_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_INT_OIS,
					(uint8_t *)&int_ois, 1);
	}

	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_CTRL1_OIS,
				       (uint8_t *)&ctrl1_ois, 1);
	}
...
int32_t lsm6dso_aux_mode_set(stmdev_ctx_t *ctx, lsm6dso_ois_en_spi2_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_CTRL1_OIS, (uint8_t *)&reg,
					1);
	}
					 ...
					 lsm6dso_ftype_ois_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_CTRL2_OIS, (uint8_t *)&reg,
					1);
	}
				 ...
				 lsm6dso_st_ois_clampdis_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_CTRL3_OIS, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_int_notification_set(stmdev_ctx_t *ctx, lsm6dso_lir_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_TAP_CFG0,
					(uint8_t *)&tap_cfg0, 1);
	}
...
int32_t lsm6dso_wkup_threshold_set(stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		reg.wk_ths = val;
		ret = lsm6dso_write_reg(ctx, LSM6DSO_WAKE_UP_THS,
					(uint8_t *)&reg, 1);
	}
...
int32_t lsm6dso_act_mode_set(stmdev_ctx_t *ctx, lsm6dso_inact_en_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_TAP_CFG2, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_tap_threshold_x_set(stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		reg.tap_ths_x = val;
		ret = lsm6dso_write_reg(ctx, LSM6DSO_TAP_CFG1, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_tap_threshold_z_set(stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		reg.tap_ths_z = val;
		ret = lsm6dso_write_reg(ctx, LSM6DSO_TAP_THS_6D,
					(uint8_t *)&reg, 1);
	}
...
int32_t lsm6dso_tap_shock_set(stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		reg.shock = val;
		ret = lsm6dso_write_reg(ctx, LSM6DSO_INT_DUR2, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_ff_threshold_set(stmdev_ctx_t *ctx, lsm6dso_ff_ths_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_FREE_FALL, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_ff_dur_set(stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_FREE_FALL,
				       (uint8_t *)&free_fall, 1);
	}
	...
	if (ret == 0) {
		ret = lsm6dso_write_reg(ctx, LSM6DSO_FREE_FALL,
					(uint8_t *)&free_fall, 1);
	}
...
int32_t lsm6dso_fifo_watermark_get(stmdev_ctx_t *ctx, uint16_t *val)
{
	...
	ret = lsm6dso_read_reg(ctx, LSM6DSO_FIFO_CTRL1, (uint8_t *)&fifo_ctrl1,
			       1);

	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_FIFO_CTRL2,
				       (uint8_t *)&fifo_ctrl2, 1);
		*val = ((uint16_t)fifo_ctrl2.wtm << 8) +
		       (uint16_t)fifo_ctrl1.wtm;
	}
...
int32_t lsm6dso_fifo_xl_batch_set(stmdev_ctx_t *ctx, lsm6dso_bdr_xl_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_FIFO_CTRL3,
					(uint8_t *)&reg, 1);
	}
...
int32_t lsm6dso_fifo_mode_set(stmdev_ctx_t *ctx, lsm6dso_fifo_mode_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_FIFO_CTRL4,
					(uint8_t *)&reg, 1);
	}
...
int32_t lsm6dso_fifo_sensor_tag_get(stmdev_ctx_t *ctx, lsm6dso_fifo_tag_t *val)
{
	...
	ret = lsm6dso_read_reg(ctx, LSM6DSO_FIFO_DATA_OUT_TAG, (uint8_t *)&reg,
			       1);
...
int32_t lsm6dso_den_polarity_set(stmdev_ctx_t *ctx, lsm6dso_den_lh_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_CTRL9_XL, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_pedo_sens_get(stmdev_ctx_t *ctx, lsm6dso_pedo_md_t *val)
{
	...
	switch ((pedo_cmd_reg.ad_det_en << 5) |
		(pedo_cmd_reg.fp_rejection_en << 4)) {
...
int32_t lsm6dso_mag_offset_set(stmdev_ctx_t *ctx, int16_t *val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_ln_pg_write_byte(ctx, LSM6DSO_MAG_OFFX_H,
					       &buff[1]);
	}

	if (ret == 0) {
		ret = lsm6dso_ln_pg_write_byte(ctx, LSM6DSO_MAG_OFFY_L,
					       &buff[2]);
	}

	if (ret == 0) {
		ret = lsm6dso_ln_pg_write_byte(ctx, LSM6DSO_MAG_OFFY_H,
					       &buff[3]);
	}

	if (ret == 0) {
		ret = lsm6dso_ln_pg_write_byte(ctx, LSM6DSO_MAG_OFFZ_L,
					       &buff[4]);
	}

	if (ret == 0) {
		ret = lsm6dso_ln_pg_write_byte(ctx, LSM6DSO_MAG_OFFZ_H,
					       &buff[5]);
	}
...
int32_t lsm6dso_mag_offset_get(stmdev_ctx_t *ctx, int16_t *val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_OFFX_H,
					      &buff[1]);
	}

	if (ret == 0) {
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_OFFY_L,
					      &buff[2]);
	}

	if (ret == 0) {
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_OFFY_H,
					      &buff[3]);
	}

	if (ret == 0) {
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_OFFZ_L,
					      &buff[4]);
	}

	if (ret == 0) {
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_OFFZ_H,
					      &buff[5]);
		val[0] = (int16_t)buff[1];
		val[0] = (val[0] * 256) + (int16_t)buff[0];
		val[1] = (int16_t)buff[3];
...
int32_t lsm6dso_mag_soft_iron_get(stmdev_ctx_t *ctx, int16_t *val)
{
	...
	if (ret == 0) {
		index++;
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_SI_XX_H,
					      &buff[index]);
	}

	if (ret == 0) {
		index++;
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_SI_XY_L,
					      &buff[index]);
	}

	if (ret == 0) {
		index++;
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_SI_XY_H,
					      &buff[index]);
	}

	if (ret == 0) {
		index++;
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_SI_XZ_L,
					      &buff[index]);
	}

	if (ret == 0) {
		index++;
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_SI_XZ_H,
					      &buff[index]);
	}

	if (ret == 0) {
		index++;
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_SI_YY_L,
					      &buff[index]);
	}

	if (ret == 0) {
		index++;
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_SI_YY_H,
					      &buff[index]);
	}

	if (ret == 0) {
		index++;
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_SI_YZ_L,
					      &buff[index]);
	}

	if (ret == 0) {
		index++;
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_SI_YZ_H,
					      &buff[index]);
	}

	if (ret == 0) {
		index++;
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_SI_ZZ_L,
					      &buff[index]);
	}

	if (ret == 0) {
		index++;
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_MAG_SI_ZZ_H,
					      &buff[index]);
	}
...
int32_t lsm6dso_fsm_enable_get(stmdev_ctx_t *ctx, lsm6dso_emb_fsm_enable_t *val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_FSM_ENABLE_A,
				       (uint8_t *)val, 2);
	}
...
int32_t lsm6dso_long_cnt_set(stmdev_ctx_t *ctx, uint16_t val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_write_reg(ctx, LSM6DSO_FSM_LONG_COUNTER_L, buff,
					2);
	}
...
int32_t lsm6dso_long_cnt_get(stmdev_ctx_t *ctx, uint16_t *val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_FSM_LONG_COUNTER_L, buff,
				       2);
	}
...
int32_t lsm6dso_fsm_out_get(stmdev_ctx_t *ctx, lsm6dso_fsm_out_t *val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_FSM_OUTS1, (uint8_t *)val,
				       16);
	}
...
int32_t lsm6dso_fsm_start_address_get(stmdev_ctx_t *ctx, uint16_t *val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_ln_pg_read_byte(ctx, LSM6DSO_FSM_START_ADD_H,
					      &buff[1]);
		*val = buff[1];
		*val = (*val * 256U) + buff[0];
	}
				     ...
				     uint8_t len)
{
	...
	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_SENSOR_HUB_1,
				       (uint8_t *)val, len);
	}
				       ...
				       lsm6dso_aux_sens_on_t val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_MASTER_CONFIG,
				       (uint8_t *)&reg, 1);
	}

	if (ret == 0) {
		reg.aux_sens_on = (uint8_t)val;
		ret = lsm6dso_write_reg(ctx, LSM6DSO_MASTER_CONFIG,
					(uint8_t *)&reg, 1);
	}
...
int32_t lsm6dso_sh_cfg_write(stmdev_ctx_t *ctx, lsm6dso_sh_cfg_write_t *val)
{
	...
	if (ret == 0) {
		...
		reg.rw_0 = 0;
		ret = lsm6dso_write_reg(ctx, LSM6DSO_SLV0_ADD, (uint8_t *)&reg,
					1);
	}
...
int32_t lsm6dso_sh_slv0_cfg_read(stmdev_ctx_t *ctx, lsm6dso_sh_cfg_read_t *val)
{
	...
	if (ret == 0) {
		...
		slv0_add.rw_0 = 1;
		ret = lsm6dso_write_reg(ctx, LSM6DSO_SLV0_ADD,
					(uint8_t *)&slv0_add, 1);
	}
...
/**
  ...
  *
  */
int32_t lsm6dso_sh_slv1_cfg_read(stmdev_ctx_t *ctx, lsm6dso_sh_cfg_read_t *val)
{
	...
	if (ret == 0) {
		...
		slv1_add.r_1 = 1;
		ret = lsm6dso_write_reg(ctx, LSM6DSO_SLV1_ADD,
					(uint8_t *)&slv1_add, 1);
	}
...
/**
  ...
  *
  */
int32_t lsm6dso_sh_slv2_cfg_read(stmdev_ctx_t *ctx, lsm6dso_sh_cfg_read_t *val)
{
	...
	if (ret == 0) {
		...
		slv2_add.r_2 = 1;
		ret = lsm6dso_write_reg(ctx, LSM6DSO_SLV2_ADD,
					(uint8_t *)&slv2_add, 1);
	}
...
/**
  ...
  *
  */
int32_t lsm6dso_sh_slv3_cfg_read(stmdev_ctx_t *ctx, lsm6dso_sh_cfg_read_t *val)
{
	...
	if (ret == 0) {
		...
		slv3_add.r_3 = 1;
		ret = lsm6dso_write_reg(ctx, LSM6DSO_SLV3_ADD,
					(uint8_t *)&slv3_add, 1);
	}
...
int32_t lsm6dso_sh_status_get(stmdev_ctx_t *ctx, lsm6dso_status_master_t *val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_STATUS_MASTER,
				       (uint8_t *)val, 1);
	}
...
int32_t lsm6dso_init_set(stmdev_ctx_t *ctx, lsm6dso_init_t val)
{
	...
	if (ret == 0) {
		emb_func_init_b.fifo_compr_init =
			(uint8_t)val & ((uint8_t)LSM6DSO_FIFO_COMP >> 2);
	...
	if (ret == 0) {
		emb_func_init_a.step_det_init =
			((uint8_t)val & (uint8_t)LSM6DSO_PEDO) >> 5;
		emb_func_init_a.tilt_init =
			((uint8_t)val & (uint8_t)LSM6DSO_TILT) >> 6;
		emb_func_init_a.sig_mot_init =
			((uint8_t)val & (uint8_t)LSM6DSO_SMOTION) >> 7;
...
int32_t lsm6dso_pin_conf_set(stmdev_ctx_t *ctx, lsm6dso_pin_conf_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_PIN_CTRL,
					(uint8_t *)&pin_ctrl, 1);
	}
...
int32_t lsm6dso_interrupt_mode_set(stmdev_ctx_t *ctx, lsm6dso_int_mode_t val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_TAP_CFG0,
				       (uint8_t *)&tap_cfg0, 1);
	}
				   ...
				   lsm6dso_pin_int1_route_t val)
{
	...
	if (ret == 0) {
		if ((emb_func_int1.int1_fsm_lc | emb_func_int1.int1_sig_mot |
		     ...
		     emb_func_int1.int1_tilt | fsm_int1_a.int1_fsm1 |
		     fsm_int1_a.int1_fsm2 | fsm_int1_a.int1_fsm3 |
		     fsm_int1_a.int1_fsm4 | fsm_int1_a.int1_fsm5 |
		     fsm_int1_a.int1_fsm6 | fsm_int1_a.int1_fsm7 |
		     fsm_int1_a.int1_fsm8 | fsm_int1_b.int1_fsm9 |
		     fsm_int1_b.int1_fsm10 | fsm_int1_b.int1_fsm11 |
		     fsm_int1_b.int1_fsm12 | fsm_int1_b.int1_fsm13 |
		     fsm_int1_b.int1_fsm14 | fsm_int1_b.int1_fsm15 |
	...
	if (ret == 0) {
		ret = lsm6dso_write_reg(ctx, LSM6DSO_MD1_CFG,
					(uint8_t *)&md1_cfg, 1);
	}
	...
	if (ret == 0) {
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_MD2_CFG,
					(uint8_t *)&md2_cfg, 1);
	}

	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_TAP_CFG2,
				       (uint8_t *)&tap_cfg2, 1);
	}
	...
	if (ret == 0) {
		if ((pin_int2_route.fifo_bdr | pin_int2_route.drdy_g |
		     ...
		     int1_ctrl.int1_fifo_th | md1_cfg.int1_shub |
		     md1_cfg.int1_6d | md1_cfg.int1_double_tap |
		     md1_cfg.int1_ff | md1_cfg.int1_wu |
		...
		ret = lsm6dso_write_reg(ctx, LSM6DSO_TAP_CFG2,
					(uint8_t *)&tap_cfg2, 1);
	}
				   ...
				   lsm6dso_pin_int1_route_t *val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_MD1_CFG,
				       (uint8_t *)&md1_cfg, 1);
	}
				   ...
				   lsm6dso_pin_int2_route_t val)
{
	...
	if (ctx != NULL) {
		...
		if (ret == 0) {
			...
			if (ret == 0) {
				if ((val.drdy_temp | val.timestamp) !=
				    PROPERTY_DISABLE) {
					ctrl4_c.int2_on_int1 = PROPERTY_DISABLE;
				}
		...
		if (ret == 0) {
			ret = lsm6dso_mem_bank_set(ctx,
						   LSM6DSO_EMBEDDED_FUNC_BANK);
		}
		...
		if (ret == 0) {
			if ((emb_func_int2.int2_fsm_lc |
			     ...
			     emb_func_int2.int2_tilt | fsm_int2_a.int2_fsm1 |
			     fsm_int2_a.int2_fsm2 | fsm_int2_a.int2_fsm3 |
			     fsm_int2_a.int2_fsm4 | fsm_int2_a.int2_fsm5 |
			     fsm_int2_a.int2_fsm6 | fsm_int2_a.int2_fsm7 |
			     fsm_int2_a.int2_fsm8 | fsm_int2_b.int2_fsm9 |
			     fsm_int2_b.int2_fsm10 | fsm_int2_b.int2_fsm11 |
			     fsm_int2_b.int2_fsm12 | fsm_int2_b.int2_fsm13 |
			     fsm_int2_b.int2_fsm14 | fsm_int2_b.int2_fsm15 |
		...
		if (ret == 0) {
			if ((val.fifo_bdr | val.drdy_g | val.drdy_temp |
			     val.drdy_xl | val.fifo_full | val.fifo_ovr |
			     val.fifo_th | val.six_d | val.double_tap |
			     val.free_fall | val.wake_up | val.single_tap |
			     val.sleep_change | pin_int1_route.den_flag |
			     pin_int1_route.boot | pin_int1_route.fifo_bdr |
			     pin_int1_route.drdy_g | pin_int1_route.drdy_xl |
...
int32_t lsm6dso_all_sources_get(stmdev_ctx_t *ctx, lsm6dso_all_sources_t *val)
{
	...
	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_EMB_FUNC_STATUS_MAINPAGE,
				       reg, 3);
	}
	...
	if (ret == 0) {
		ret = lsm6dso_read_reg(ctx, LSM6DSO_STATUS_MASTER_MAINPAGE, reg,
				       3);
	}
			 ...
			 lsm6dso_md_t *val)
{
	...
	if (aux_ctx != NULL) {
		if (ret == 0) {
			ret = lsm6dso_read_reg(aux_ctx, LSM6DSO_CTRL1_OIS, reg,
					       3);
		}
	...
	if (val->fsm.sens != LSM6DSO_FSM_DISABLE) {
		...
		case LSM6DSO_FSM_12Hz5:
			if ((val->fsm.sens != LSM6DSO_FSM_GY) &&
			    (odr_xl == 0x00U)) {
				odr_xl = 0x01U;
			}

			if ((val->fsm.sens != LSM6DSO_FSM_XL) &&
			    (odr_gy == 0x00U)) {
	...
	if (aux_ctx != NULL) {
		...
		if (ret == 0) {
			ret = lsm6dso_write_reg(aux_ctx, LSM6DSO_CTRL1_OIS, reg,
						3);
		}
	}
			 ...
			 lsm6dso_md_t *val)
{
	...
	if (ctx != NULL) {
		...
		if (ret == 0) {
			ret = lsm6dso_read_reg(ctx, LSM6DSO_FSM_ENABLE_A, reg,
					       2);
	...
	if ((fsm_enable_a.fsm1_en | fsm_enable_a.fsm2_en |
	     fsm_enable_a.fsm3_en | fsm_enable_a.fsm4_en |
	     fsm_enable_a.fsm5_en | fsm_enable_a.fsm6_en |
	     fsm_enable_a.fsm7_en | fsm_enable_a.fsm8_en |
	     fsm_enable_b.fsm9_en | fsm_enable_b.fsm10_en |
	     fsm_enable_b.fsm11_en | fsm_enable_b.fsm12_en |
	     fsm_enable_b.fsm13_en | fsm_enable_b.fsm14_en |
	     fsm_enable_b.fsm15_en | fsm_enable_b.fsm16_en) ==
	    PROPERTY_ENABLE) {
			 ...
			 lsm6dso_md_t *md, lsm6dso_data_t *data)
{
	...
	j += 2U;
	data->ui.heat.deg_c =
		lsm6dso_from_lsb_to_celsius((int16_t)data->ui.heat.raw);

	/* angular rate conversion */
	for (i = 0U; i < 3U; i++) {
		data->ui.gy.raw[i] = (int16_t)buff[j + 1U];
		data->ui.gy.raw[i] =
			(data->ui.gy.raw[i] * 256) + (int16_t)buff[j];
		j += 2U;

		switch (md->ui.gy.fs) {
		case LSM6DSO_GY_UI_250dps:
			data->ui.gy.mdps[i] =
				lsm6dso_from_fs250_to_mdps(data->ui.gy.raw[i]);
			break;

		case LSM6DSO_GY_UI_125dps:
			data->ui.gy.mdps[i] =
				lsm6dso_from_fs125_to_mdps(data->ui.gy.raw[i]);
			break;

		case LSM6DSO_GY_UI_500dps:
			data->ui.gy.mdps[i] =
				lsm6dso_from_fs500_to_mdps(data->ui.gy.raw[i]);
			break;

		case LSM6DSO_GY_UI_1000dps:
			data->ui.gy.mdps[i] =
				lsm6dso_from_fs1000_to_mdps(data->ui.gy.raw[i]);
			break;

		case LSM6DSO_GY_UI_2000dps:
			data->ui.gy.mdps[i] =
				lsm6dso_from_fs2000_to_mdps(data->ui.gy.raw[i]);
			break;
	...
	for (i = 0U; i < 3U; i++) {
		...
		data->ui.xl.raw[i] =
			(data->ui.xl.raw[i] * 256) + (int16_t)buff[j];
		j += 2U;

		switch (md->ui.xl.fs) {
		case LSM6DSO_XL_UI_2g:
			data->ui.xl.mg[i] =
				lsm6dso_from_fs2_to_mg(data->ui.xl.raw[i]);
			break;

		case LSM6DSO_XL_UI_4g:
			data->ui.xl.mg[i] =
				lsm6dso_from_fs4_to_mg(data->ui.xl.raw[i]);
			break;

		case LSM6DSO_XL_UI_8g:
			data->ui.xl.mg[i] =
				lsm6dso_from_fs8_to_mg(data->ui.xl.raw[i]);
			break;

		case LSM6DSO_XL_UI_16g:
			data->ui.xl.mg[i] =
				lsm6dso_from_fs16_to_mg(data->ui.xl.raw[i]);
			break;
	...
	if (aux_ctx != NULL) {
		if (ret == 0) {
			ret = lsm6dso_read_reg(aux_ctx, LSM6DSO_OUTX_L_G, buff,
					       12);
		}
	}
	...
	for (i = 0U; i < 3U; i++) {
		...
		data->ois.gy.raw[i] =
			(data->ois.gy.raw[i] * 256) + (int16_t)buff[j];
		j += 2U;

		switch (md->ois.gy.fs) {
		case LSM6DSO_GY_UI_250dps:
			data->ois.gy.mdps[i] =
				lsm6dso_from_fs250_to_mdps(data->ois.gy.raw[i]);
			break;

		case LSM6DSO_GY_UI_125dps:
			data->ois.gy.mdps[i] =
				lsm6dso_from_fs125_to_mdps(data->ois.gy.raw[i]);
			break;

		case LSM6DSO_GY_UI_500dps:
			data->ois.gy.mdps[i] =
				lsm6dso_from_fs500_to_mdps(data->ois.gy.raw[i]);
			break;
	...
	for (i = 0U; i < 3U; i++) {
		...
		data->ois.xl.raw[i] =
			(data->ois.xl.raw[i] * 256) + (int16_t)buff[j];
		j += 2U;

		switch (md->ois.xl.fs) {
		case LSM6DSO_XL_UI_2g:
			data->ois.xl.mg[i] =
				lsm6dso_from_fs2_to_mg(data->ois.xl.raw[i]);
			break;

		case LSM6DSO_XL_UI_4g:
			data->ois.xl.mg[i] =
				lsm6dso_from_fs4_to_mg(data->ois.xl.raw[i]);
			break;

		case LSM6DSO_XL_UI_8g:
			data->ois.xl.mg[i] =
				lsm6dso_from_fs8_to_mg(data->ois.xl.raw[i]);
			break;

		case LSM6DSO_XL_UI_16g:
			data->ois.xl.mg[i] =
				lsm6dso_from_fs16_to_mg(data->ois.xl.raw[i]);
			break;
