...
static uint32_t ToFloatBits(uint16_t h)
{
	...
	switch (h_exp) {
		...
		case 0x7c00u: // inf or NaN
			...
			return f_sgn + 0x7f800000u +
			       (((uint32_t)(h & 0x03ffu)) << 13);
		default: // normalized
			// Just need to adjust the exponent and shift
			return f_sgn +
			       (((uint32_t)(h & 0x7fffu) + 0x1c000u) << 13);
	}
}
...
int32_t lsm6dsv_reset_get(const stmdev_ctx_t *ctx, lsm6dsv_reset_t *val)
{
	...
	switch ((ctrl3.sw_reset << 2) + (ctrl3.boot << 1) +
		func_cfg_access.sw_por) {
		case LSM6DSV_READY:
			*val = LSM6DSV_READY;
			break;
				     ...
				     lsm6dsv_embedded_int_config_t val)
{
	...
	ret = lsm6dsv_mem_bank_set(ctx, LSM6DSV_EMBED_FUNC_MEM_BANK);
	if (ret == 0) {
		ret = lsm6dsv_read_reg(ctx, LSM6DSV_PAGE_RW,
				       (uint8_t *)&page_rw, 1);
