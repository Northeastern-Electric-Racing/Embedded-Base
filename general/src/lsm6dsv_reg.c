...
int32_t lsm6dsv_xl_offset_on_out_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_CTRL9, (uint8_t *)&ctrl9,
					1);
	}
				 ...
				 lsm6dsv_xl_offset_mg_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_Z_OFS_USR, (uint8_t *)&z_ofs_usr,
			       1);
	ret += lsm6dsv_read_reg(ctx, LSM6DSV_Y_OFS_USR, (uint8_t *)&y_ofs_usr,
				1);
	ret += lsm6dsv_read_reg(ctx, LSM6DSV_X_OFS_USR, (uint8_t *)&x_ofs_usr,
				1);
	if (ret != 0) {
		return ret;
	}
	...
	ret = lsm6dsv_write_reg(ctx, LSM6DSV_Z_OFS_USR, (uint8_t *)&z_ofs_usr,
				1);
	ret += lsm6dsv_write_reg(ctx, LSM6DSV_Y_OFS_USR, (uint8_t *)&y_ofs_usr,
				 1);
	ret += lsm6dsv_write_reg(ctx, LSM6DSV_X_OFS_USR, (uint8_t *)&x_ofs_usr,
				 1);
...
int32_t lsm6dsv_sw_por(const stmdev_ctx_t *ctx)
{
	...
	ret = lsm6dsv_write_reg(ctx, LSM6DSV_FUNC_CFG_ACCESS,
...
int32_t lsm6dsv_sw_reset(const stmdev_ctx_t *ctx)
{
	...
	do {
		ret += lsm6dsv_read_reg(ctx, LSM6DSV_CTRL3, (uint8_t *)&ctrl3,
					1);
		if (ret != 0) {
			goto exit;
		}
...
int32_t lsm6dsv_mem_bank_set(const stmdev_ctx_t *ctx, lsm6dsv_mem_bank_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_FUNC_CFG_ACCESS,
			       (uint8_t *)&func_cfg_access, 1);
	if (ret != 0) {
		return ret;
	}
				 ...
				 lsm6dsv_data_rate_t val)
{
	...
	if (sel != 0U) {
		ret += lsm6dsv_read_reg(ctx, LSM6DSV_HAODR_CFG,
					(uint8_t *)&haodr, 1);
		if (ret != 0) {
			return ret;
		}
		...
		haodr.haodr_sel = sel;
		ret += lsm6dsv_write_reg(ctx, LSM6DSV_HAODR_CFG,
					 (uint8_t *)&haodr, 1);
	}
...
int32_t lsm6dsv_xl_mode_set(const stmdev_ctx_t *ctx, lsm6dsv_xl_mode_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_CTRL1, (uint8_t *)&ctrl1,
					1);
	}
...
int32_t lsm6dsv_gy_mode_set(const stmdev_ctx_t *ctx, lsm6dsv_gy_mode_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_CTRL2, (uint8_t *)&ctrl2,
					1);
	}
...
int32_t lsm6dsv_auto_increment_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		ctrl3.if_inc = val;
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_CTRL3, (uint8_t *)&ctrl3,
					1);
	}
...
int32_t lsm6dsv_odr_trig_cfg_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_ODR_TRIG_CFG, (uint8_t *)&odr_trig,
			       1);

	if (ret == 0) {
		odr_trig.odr_trig_nodr = val;
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_ODR_TRIG_CFG,
					(uint8_t *)&odr_trig, 1);
	}
				    ...
				    lsm6dsv_data_ready_mode_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_CTRL4, (uint8_t *)&ctrl4,
					1);
	}
				     ...
				     lsm6dsv_interrupt_mode_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_FUNCTIONS_ENABLE, (uint8_t *)&func,
			       1);
	if (ret != 0) {
		return ret;
	}
	func.interrupts_enable = val.enable;
	ret = lsm6dsv_write_reg(ctx, LSM6DSV_FUNCTIONS_ENABLE, (uint8_t *)&func,
				1);
				  ...
				  lsm6dsv_gy_full_scale_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_CTRL6, (uint8_t *)&ctrl6,
					1);
	}
				  ...
				  lsm6dsv_xl_full_scale_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_CTRL8, (uint8_t *)&ctrl8,
					1);
	}
				 ...
				 lsm6dsv_xl_self_test_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_CTRL10, (uint8_t *)&ctrl10,
					1);
	}
				     ...
				     lsm6dsv_ois_xl_self_test_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_SPI2_INT_OIS,
			       (uint8_t *)&spi2_int_ois, 1);

	if (ret == 0) {
		spi2_int_ois.st_xl_ois = ((uint8_t)val & 0x3U);
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_SPI2_INT_OIS,
					(uint8_t *)&spi2_int_ois, 1);
	}
				     ...
				     lsm6dsv_ois_gy_self_test_t *val)
{
	...
	case LSM6DSV_OIS_GY_ST_POSITIVE:
		*val = (spi2_int_ois.st_ois_clampdis == 1U) ?
			       LSM6DSV_OIS_GY_ST_CLAMP_POS :
			       LSM6DSV_OIS_GY_ST_POSITIVE;
		break;

	case LSM6DSV_OIS_GY_ST_NEGATIVE:
		*val = (spi2_int_ois.st_ois_clampdis == 1U) ?
			       LSM6DSV_OIS_GY_ST_CLAMP_NEG :
				   ...
				   lsm6dsv_pin_int_route_t *val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_INT1_CTRL, (uint8_t *)&int1_ctrl,
			       1);
	if (ret != 0) {
		return ret;
	}
				   ...
				   lsm6dsv_pin_int_route_t *val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_INT2_CTRL, (uint8_t *)&int2_ctrl,
			       1);
	if (ret != 0) {
		return ret;
	}
				       ...
				       const lsm6dsv_emb_pin_int_route_t *val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_EMB_FUNC_INT1,
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_EMB_FUNC_INT1,
					(uint8_t *)&emb_func_int1, 1);
	}
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_MD1_CFG,
					(uint8_t *)&md1_cfg, 1);
	}
				       ...
				       const lsm6dsv_emb_pin_int_route_t *val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_EMB_FUNC_INT2,
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_EMB_FUNC_INT2,
					(uint8_t *)&emb_func_int2, 1);
	}
	ret += lsm6dsv_mem_bank_set(ctx, LSM6DSV_MAIN_MEM_BANK);
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_MD2_CFG,
					(uint8_t *)&md2_cfg, 1);
	}
				     ...
				     lsm6dsv_embedded_int_config_t val)
{
	...
	if (ret == 0) {
		...
		ret += lsm6dsv_write_reg(ctx, LSM6DSV_PAGE_RW,
					 (uint8_t *)&page_rw, 1);
	}
				    ...
				    lsm6dsv_embedded_status_t *val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_EMB_FUNC_STATUS_MAINPAGE,
	...
	ret += lsm6dsv_read_reg(ctx, LSM6DSV_EMB_FUNC_SRC,
				...
				lsm6dsv_all_sources_t *val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_FUNCTIONS_ENABLE,
			       (uint8_t *)&functions_enable, 1);
	if (ret != 0) {
		return ret;
	}
	functions_enable.dis_rst_lir_all_int = PROPERTY_ENABLE;
	ret = lsm6dsv_write_reg(ctx, LSM6DSV_FUNCTIONS_ENABLE,
				(uint8_t *)&functions_enable, 1);
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_UI_STATUS_REG_OIS, (uint8_t *)&buff,
			       8);
	if (ret != 0) {
		return ret;
	}
	...
	/* sensor hub */
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_STATUS_MASTER_MAINPAGE,
...
int32_t lsm6dsv_odr_cal_reg_get(const stmdev_ctx_t *ctx, int8_t *val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_INTERNAL_FREQ,
			    ...
			    uint8_t *buf, uint8_t len)
{
	...
	ret += lsm6dsv_write_reg(ctx, LSM6DSV_PAGE_SEL, (uint8_t *)&page_sel,
				 1);
	if (ret != 0) {
		goto exit;
	}
	...
	for (i = 0; i < len; i++) {
		...
		if (((lsb & 0xFFU) == 0x00U) && (ret == 0)) {
			msb++;
			ret += lsm6dsv_read_reg(ctx, LSM6DSV_PAGE_SEL,
						(uint8_t *)&page_sel, 1);
			if (ret != 0) {
				goto exit;
			}
			...
			page_sel.page_sel = msb;
			page_sel.not_used0 = 1; // Default value
			ret += lsm6dsv_write_reg(ctx, LSM6DSV_PAGE_SEL,
						 (uint8_t *)&page_sel, 1);
			if (ret != 0) {
				goto exit;
			}
...
/**
  ...
  *
  */
int32_t lsm6dsv_ln_pg_read(const stmdev_ctx_t *ctx, uint16_t address,
			   uint8_t *buf, uint8_t len)
{
	lsm6dsv_page_address_t page_address;
	lsm6dsv_page_sel_t page_sel;
...
int32_t lsm6dsv_den_conf_get(const stmdev_ctx_t *ctx, lsm6dsv_den_conf_t *val)
{
	...
	if ((den.den_x | den.den_y | den.den_z) == PROPERTY_ENABLE) {
		if (den.den_xl_g == PROPERTY_DISABLE &&
		    den.den_xl_en == PROPERTY_ENABLE) {
			val->stamp_in_gy_data = PROPERTY_ENABLE;
			val->stamp_in_xl_data = PROPERTY_ENABLE;
		} else if (den.den_xl_g == PROPERTY_DISABLE &&
			   den.den_xl_en == PROPERTY_DISABLE) {
			val->stamp_in_gy_data = PROPERTY_ENABLE;
			val->stamp_in_xl_data = PROPERTY_DISABLE;
				      ...
				      lsm6dsv_eis_gy_full_scale_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_CTRL_EIS,
					(uint8_t *)&ctrl_eis, 1);
	}
...
int32_t lsm6dsv_fifo_watermark_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	fifo_ctrl1.wtm = val;
	ret = lsm6dsv_write_reg(ctx, LSM6DSV_FIFO_CTRL1, (uint8_t *)&fifo_ctrl1,
				1);
...
int32_t lsm6dsv_fifo_watermark_get(const stmdev_ctx_t *ctx, uint8_t *val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_FIFO_CTRL1, (uint8_t *)&fifo_ctrl1,
			       1);
	if (ret != 0) {
		return ret;
	}
...
int32_t lsm6dsv_fifo_xl_dual_fsm_batch_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_FIFO_CTRL2, (uint8_t *)&fifo_ctrl2,
			       1);
	if (ret == 0) {
		fifo_ctrl2.xl_dualc_batch_from_fsm = val;
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_FIFO_CTRL2,
...
/**
  ...
  *
  */
int32_t lsm6dsv_fifo_xl_dual_fsm_batch_get(const stmdev_ctx_t *ctx,
					   uint8_t *val)
{
	lsm6dsv_fifo_ctrl2_t fifo_ctrl2;
	int32_t ret;
						 ...
						 uint8_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_EMB_FUNC_EN_B,
			       (uint8_t *)&emb_func_en_b, 1);
	if (ret == 0) {
		emb_func_en_b.fifo_compr_en = val;
		ret += lsm6dsv_write_reg(ctx, LSM6DSV_EMB_FUNC_EN_B,
					 (uint8_t *)&emb_func_en_b, 1);
	}
	ret += lsm6dsv_mem_bank_set(ctx, LSM6DSV_MAIN_MEM_BANK);
...
/**
  ...
  *
  */
int32_t lsm6dsv_fifo_stop_on_wtm_set(const stmdev_ctx_t *ctx,
				     lsm6dsv_fifo_event_t val)
{
	lsm6dsv_fifo_ctrl2_t fifo_ctrl2;
	int32_t ret;
...
/**
  ...
  *
  */
int32_t lsm6dsv_fifo_stop_on_wtm_get(const stmdev_ctx_t *ctx,
				     lsm6dsv_fifo_event_t *val)
{
	lsm6dsv_fifo_ctrl2_t fifo_ctrl2;
	int32_t ret;
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_FIFO_CTRL2, (uint8_t *)&fifo_ctrl2,
			       1);
	if (ret == 0) {
		*val = (fifo_ctrl2.stop_on_wtm == 1) ? LSM6DSV_FIFO_EV_WTM :
						       LSM6DSV_FIFO_EV_FULL;
	}
				  ...
				  lsm6dsv_fifo_xl_batch_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_FIFO_CTRL3, (uint8_t *)&fifo_ctrl3,
			       1);
	if (ret == 0) {
		fifo_ctrl3.bdr_xl = (uint8_t)val & 0xFu;
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_FIFO_CTRL3,
...
int32_t lsm6dsv_fifo_mode_set(const stmdev_ctx_t *ctx, lsm6dsv_fifo_mode_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_FIFO_CTRL4, (uint8_t *)&fifo_ctrl4,
			       1);
	if (ret == 0) {
		fifo_ctrl4.fifo_mode = (uint8_t)val & 0x07U;
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_FIFO_CTRL4,
						 ...
						 uint16_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_COUNTER_BDR_REG1,
			       (uint8_t *)&counter_bdr_reg1, 1);

	if (ret == 0) {
		counter_bdr_reg2.cnt_bdr_th = (uint8_t)val & 0xFFU;
		counter_bdr_reg1.cnt_bdr_th = (uint8_t)(val >> 8) & 0x3U;
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_COUNTER_BDR_REG1,
					(uint8_t *)&counter_bdr_reg1, 1);
		ret += lsm6dsv_write_reg(ctx, LSM6DSV_COUNTER_BDR_REG2,
				...
				lsm6dsv_fifo_status_t *val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_FIFO_STATUS1, (uint8_t *)&buff[0],
			       2);
	if (ret != 0) {
		return ret;
	}
...
int32_t lsm6dsv_fifo_stpcnt_batch_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_EMB_FUNC_FIFO_EN_A,
...
/**
  ...
  *
  */
int32_t lsm6dsv_fifo_sh_batch_slave_set(const stmdev_ctx_t *ctx, uint8_t idx,
					uint8_t val)
{
...
/**
  ...
  *
  */
int32_t lsm6dsv_fifo_sh_batch_slave_get(const stmdev_ctx_t *ctx, uint8_t idx,
					uint8_t *val)
{
	lsm6dsv_slv0_config_t slv_config;
	int32_t ret;
				    ...
				    lsm6dsv_filt_anti_spike_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_IF_CFG, (uint8_t *)&if_cfg,
					1);
	}
				       ...
				       lsm6dsv_filt_settling_mask_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_EMB_FUNC_CFG,
	...
	ret += lsm6dsv_write_reg(ctx, LSM6DSV_EMB_FUNC_CFG,
				 (uint8_t *)&emb_func_cfg, 1);
	if (ret != 0) {
		return ret;
	}

	ret = lsm6dsv_read_reg(ctx, LSM6DSV_UI_INT_OIS, (uint8_t *)&ui_int_ois,
			       1);
	if (ret != 0) {
		return ret;
	}
	ui_int_ois.drdy_mask_ois = val.ois_drdy;
	ret += lsm6dsv_write_reg(ctx, LSM6DSV_UI_INT_OIS,
...
/**
  ...
  */
int32_t
lsm6dsv_filt_ois_settling_mask_get(const stmdev_ctx_t *ctx,
				   lsm6dsv_filt_ois_settling_mask_t *val)
{
	lsm6dsv_spi2_int_ois_t spi2_int_ois;
	int32_t ret;
...
int32_t lsm6dsv_filt_gy_lp1_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		ctrl7.lpf1_g_en = val;
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_CTRL7, (uint8_t *)&ctrl7,
					1);
	}
				       ...
				       lsm6dsv_filt_wkup_act_feed_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_WAKE_UP_THS,
	...
	ret = lsm6dsv_write_reg(ctx, LSM6DSV_WAKE_UP_THS,
				(uint8_t *)&wake_up_ths, 1);
...
int32_t lsm6dsv_mask_trigger_xl_settl_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_TAP_CFG0,
					(uint8_t *)&tap_cfg0, 1);
	}
...
/**
  ...
  */
int32_t
lsm6dsv_filt_gy_eis_lp_bandwidth_set(const stmdev_ctx_t *ctx,
...
/**
  ...
  */
int32_t
lsm6dsv_filt_gy_eis_lp_bandwidth_get(const stmdev_ctx_t *ctx,
				     lsm6dsv_filt_gy_eis_lp_bandwidth_t *val)
{
	lsm6dsv_ctrl_eis_t ctrl_eis;
	int32_t ret;
...
/**
  ...
  */
int32_t
lsm6dsv_filt_gy_ois_lp_bandwidth_set(const stmdev_ctx_t *ctx,
				     lsm6dsv_filt_gy_ois_lp_bandwidth_t val)
{
	lsm6dsv_ui_ctrl2_ois_t ui_ctrl2_ois;
	int32_t ret;

	ret = lsm6dsv_read_reg(ctx, LSM6DSV_UI_CTRL2_OIS,
			       (uint8_t *)&ui_ctrl2_ois, 1);

	if (ret == 0) {
		ui_ctrl2_ois.lpf1_g_ois_bw = (uint8_t)val & 0x03U;
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_UI_CTRL2_OIS,
					(uint8_t *)&ui_ctrl2_ois, 1);
	}
...
/**
  ...
  */
int32_t
lsm6dsv_filt_gy_ois_lp_bandwidth_get(const stmdev_ctx_t *ctx,
				     lsm6dsv_filt_gy_ois_lp_bandwidth_t *val)
{
	lsm6dsv_ui_ctrl2_ois_t ui_ctrl2_ois;
	int32_t ret;
...
/**
  ...
  */
int32_t
lsm6dsv_filt_xl_ois_lp_bandwidth_set(const stmdev_ctx_t *ctx,
				     lsm6dsv_filt_xl_ois_lp_bandwidth_t val)
{
	lsm6dsv_ui_ctrl3_ois_t ui_ctrl3_ois;
	int32_t ret;

	ret = lsm6dsv_read_reg(ctx, LSM6DSV_UI_CTRL3_OIS,
			       (uint8_t *)&ui_ctrl3_ois, 1);

	if (ret == 0) {
		ui_ctrl3_ois.lpf_xl_ois_bw = (uint8_t)val & 0x07U;
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_UI_CTRL3_OIS,
					(uint8_t *)&ui_ctrl3_ois, 1);
	}
...
/**
  ...
  */
int32_t
lsm6dsv_filt_xl_ois_lp_bandwidth_get(const stmdev_ctx_t *ctx,
				     lsm6dsv_filt_xl_ois_lp_bandwidth_t *val)
{
	lsm6dsv_ui_ctrl3_ois_t ui_ctrl3_ois;
	int32_t ret;
...
int32_t lsm6dsv_fsm_permission_status(const stmdev_ctx_t *ctx, uint8_t *val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_CTRL_STATUS,
...
int32_t lsm6dsv_fsm_mode_set(const stmdev_ctx_t *ctx, lsm6dsv_fsm_mode_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_EMB_FUNC_EN_B,
			       (uint8_t *)&emb_func_en_b, 1);
	ret += lsm6dsv_read_reg(ctx, LSM6DSV_FSM_ENABLE, (uint8_t *)&fsm_enable,
				1);
	if (ret != 0) {
		goto exit;
	}
	...
	if ((val.fsm1_en | val.fsm2_en | val.fsm3_en | val.fsm4_en |
	     val.fsm5_en | val.fsm6_en | val.fsm7_en | val.fsm8_en) ==
	    PROPERTY_ENABLE) {
	...
	ret = lsm6dsv_write_reg(ctx, LSM6DSV_FSM_ENABLE, (uint8_t *)&fsm_enable,
				1);
	ret += lsm6dsv_write_reg(ctx, LSM6DSV_EMB_FUNC_EN_B,
				 (uint8_t *)&emb_func_en_b, 1);
...
int32_t lsm6dsv_fsm_long_cnt_set(const stmdev_ctx_t *ctx, uint16_t val)
{
	...
	ret += lsm6dsv_write_reg(ctx, LSM6DSV_FSM_LONG_COUNTER_L,
				 (uint8_t *)&buff[0], 2);
	ret += lsm6dsv_mem_bank_set(ctx, LSM6DSV_MAIN_MEM_BANK);
				  ...
				  lsm6dsv_fsm_data_rate_t *val)
{
	...
	ret = lsm6dsv_mem_bank_set(ctx, LSM6DSV_EMBED_FUNC_MEM_BANK);
	if (ret == 0) {
		ret += lsm6dsv_read_reg(ctx, LSM6DSV_FSM_ODR,
					(uint8_t *)&fsm_odr, 1);
	}
	ret += lsm6dsv_mem_bank_set(ctx, LSM6DSV_MAIN_MEM_BANK);
	if (ret != 0) {
				    ...
				    lsm6dsv_sflp_gbias_t *val)
{
	...
	gbias_hf[0] = npy_float_to_half(val->gbias_x *
					(3.14159265358979323846f / 180.0f) / k);
	gbias_hf[1] = npy_float_to_half(val->gbias_y *
					(3.14159265358979323846f / 180.0f) / k);
	gbias_hf[2] = npy_float_to_half(val->gbias_z *
					(3.14159265358979323846f / 180.0f) / k);
	...
	ret += lsm6dsv_read_reg(ctx, LSM6DSV_EMB_FUNC_EN_A, emb_func_en_saved,
				2);
	if (ret == 0) {
		ret += lsm6dsv_write_reg(ctx, LSM6DSV_EMB_FUNC_EN_A, reg_zero,
					 2);
	}
	if (ret == 0) {
		do {
			ret += lsm6dsv_read_reg(ctx,
						LSM6DSV_EMB_FUNC_EXEC_STATUS,
	...
	if (ret == 0) {
		...
		ret += lsm6dsv_write_reg(ctx, LSM6DSV_CTRL10,
					 (uint8_t *)&ctrl10, 1);
	}
	if (ret != 0) {
		goto exit;
	...
	for (i = 0; i < 3U; i++) {
		...
		ret += lsm6dsv_write_reg(ctx, LSM6DSV_SENSOR_HUB_2 + 3U * i,
					 &data_ptr[j++], 1);
		ret += lsm6dsv_write_reg(ctx, LSM6DSV_SENSOR_HUB_3 + 3U * i,
					 &data_ptr[j], 1);
	}
	for (i = 0; i < 3U; i++) {
		j = 0;
		...
		ret += lsm6dsv_write_reg(ctx, LSM6DSV_SENSOR_HUB_11 + 3U * i,
					 &data_ptr[j++], 1);
		ret += lsm6dsv_write_reg(ctx, LSM6DSV_SENSOR_HUB_12 + 3U * i,
					 &data_ptr[j], 1);
	}
	ret += lsm6dsv_mem_bank_set(ctx, LSM6DSV_MAIN_MEM_BANK);
	if (ret != 0) {
					     ...
					     uint16_t val)
{
	...
	buff[1] = (uint8_t)(val / 256U);
	buff[0] = (uint8_t)(val - (buff[1] * 256U));
	ret = lsm6dsv_ln_pg_write(ctx, LSM6DSV_FSM_EXT_SENSITIVITY_L,
				  (uint8_t *)&buff[0], 2);
					     ...
					     uint16_t *val)
{
	...
	ret = lsm6dsv_ln_pg_read(ctx, LSM6DSV_FSM_EXT_SENSITIVITY_L, &buff[0],
				 2);
	if (ret != 0) {
		return ret;
	}
					...
					lsm6dsv_xl_fsm_ext_sens_offset_t val)
{
	...
	ret = lsm6dsv_ln_pg_write(ctx, LSM6DSV_FSM_EXT_OFFX_L,
				  (uint8_t *)&buff[0], 6);
					...
					lsm6dsv_xl_fsm_ext_sens_matrix_t val)
{
	...
	ret = lsm6dsv_ln_pg_write(ctx, LSM6DSV_FSM_EXT_MATRIX_XX_L,
				  (uint8_t *)&buff[0], 12);
					...
					lsm6dsv_xl_fsm_ext_sens_matrix_t *val)
{
	...
	ret = lsm6dsv_ln_pg_read(ctx, LSM6DSV_FSM_EXT_MATRIX_XX_L, &buff[0],
				 12);
	if (ret != 0) {
		return ret;
	}
					  ...
					  lsm6dsv_fsm_ext_sens_z_orient_t val)
{
	...
	ret = lsm6dsv_ln_pg_read(ctx, LSM6DSV_EXT_CFG_A, (uint8_t *)&ext_cfg_a,
				 1);
	if (ret != 0) {
		return ret;
	}
	ext_cfg_a.ext_z_axis = (uint8_t)val & 0x07U;
	ret += lsm6dsv_ln_pg_write(ctx, LSM6DSV_EXT_CFG_A,
				   (uint8_t *)&ext_cfg_a, 1);
					  ...
					  lsm6dsv_fsm_ext_sens_x_orient_t val)
{
	...
	ret = lsm6dsv_ln_pg_read(ctx, LSM6DSV_EXT_CFG_B, (uint8_t *)&ext_cfg_b,
				 1);
	if (ret == 0) {
		ext_cfg_b.ext_x_axis = (uint8_t)val & 0x7U;
		ret = lsm6dsv_ln_pg_write(ctx, LSM6DSV_EXT_CFG_B,
					  (uint8_t *)&ext_cfg_b, 1);
	}
...
int32_t lsm6dsv_fsm_long_cnt_timeout_set(const stmdev_ctx_t *ctx, uint16_t val)
{
	...
	buff[1] = (uint8_t)(val / 256U);
	buff[0] = (uint8_t)(val - (buff[1] * 256U));
	ret = lsm6dsv_ln_pg_write(
		ctx, LSM6DSV_EMB_ADV_PG_1 + LSM6DSV_FSM_LC_TIMEOUT_L,
		(uint8_t *)&buff[0], 2);
...
int32_t lsm6dsv_fsm_long_cnt_timeout_get(const stmdev_ctx_t *ctx, uint16_t *val)
{
	...
	ret = lsm6dsv_ln_pg_read(
		ctx, LSM6DSV_EMB_ADV_PG_1 + LSM6DSV_FSM_LC_TIMEOUT_L, &buff[0],
		2);
	if (ret != 0) {
		return ret;
	}
...
int32_t lsm6dsv_fsm_number_of_programs_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	ret = lsm6dsv_ln_pg_read(ctx,
				 LSM6DSV_EMB_ADV_PG_1 + LSM6DSV_FSM_PROGRAMS,
				 (uint8_t *)&fsm_programs, 1);
	if (ret == 0) {
		fsm_programs.fsm_n_prog = val;
		ret = lsm6dsv_ln_pg_write(
			ctx, LSM6DSV_EMB_ADV_PG_1 + LSM6DSV_FSM_PROGRAMS,
			(uint8_t *)&fsm_programs, 1);
	}
...
/**
  ...
  *
  */
int32_t lsm6dsv_fsm_number_of_programs_get(const stmdev_ctx_t *ctx,
					   uint8_t *val)
{
	lsm6dsv_fsm_programs_t fsm_programs;
	int32_t ret;
...
int32_t lsm6dsv_fsm_start_address_set(const stmdev_ctx_t *ctx, uint16_t val)
{
	...
	buff[0] = (uint8_t)(val - (buff[1] * 256U));
	ret = lsm6dsv_ln_pg_write(
		ctx, LSM6DSV_EMB_ADV_PG_1 + LSM6DSV_FSM_START_ADD_L,
		(uint8_t *)&buff[0], 2);
...
int32_t lsm6dsv_fsm_start_address_get(const stmdev_ctx_t *ctx, uint16_t *val)
{
	...
	ret = lsm6dsv_ln_pg_read(ctx,
				 LSM6DSV_EMB_ADV_PG_1 + LSM6DSV_FSM_START_ADD_L,
				 &buff[0], 2);
	if (ret != 0) {
		return ret;
	}
...
int32_t lsm6dsv_ff_time_windows_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_WAKE_UP_DUR,
			       (uint8_t *)&wake_up_dur, 1);
	if (ret != 0) {
		return ret;
	}
	wake_up_dur.ff_dur = ((uint8_t)val & 0x20U) >> 5;
	ret = lsm6dsv_write_reg(ctx, LSM6DSV_WAKE_UP_DUR,
				(uint8_t *)&wake_up_dur, 1);

	ret += lsm6dsv_read_reg(ctx, LSM6DSV_FREE_FALL, (uint8_t *)&free_fall,
				1);
	if (ret != 0) {
		return ret;
	}
	free_fall.ff_dur = (uint8_t)val & 0x1FU;
	ret += lsm6dsv_write_reg(ctx, LSM6DSV_FREE_FALL, (uint8_t *)&free_fall,
				 1);
				  ...
				  lsm6dsv_ff_thresholds_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_FREE_FALL,
					(uint8_t *)&free_fall, 1);
	}
...
int32_t lsm6dsv_ois_interface_pull_up_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_PIN_CTRL,
					(uint8_t *)&pin_ctrl, 1);
	}
					   ...
					   lsm6dsv_ois_handshake_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_SPI2_HANDSHAKE_CTRL,
			       (uint8_t *)&spi2_handshake_ctrl, 1);
	if (ret == 0) {
		spi2_handshake_ctrl.spi2_shared_ack = val.ack;
		spi2_handshake_ctrl.spi2_shared_req = val.req;
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_SPI2_HANDSHAKE_CTRL,
					(uint8_t *)&spi2_handshake_ctrl, 1);
	}
...
int32_t lsm6dsv_ois_on_spi2_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_UI_CTRL1_OIS,
			       (uint8_t *)&ui_ctrl1_ois, 1);
	if (ret == 0) {
		ui_ctrl1_ois.spi2_read_en = val;
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_UI_CTRL1_OIS,
					(uint8_t *)&ui_ctrl1_ois, 1);
	}
				 ...
				 lsm6dsv_6d_threshold_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_TAP_THS_6D, (uint8_t *)&tap_ths_6d,
			       1);
	if (ret == 0) {
		tap_ths_6d.sixd_ths = (uint8_t)val & 0x03U;
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_TAP_THS_6D,
...
int32_t lsm6dsv_i3c_int_en_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_CTRL5, (uint8_t *)&ctrl5,
					1);
	}
				       ...
				       lsm6dsv_sh_slave_connected_t val)
{
	...
	ret = lsm6dsv_mem_bank_set(ctx, LSM6DSV_SENSOR_HUB_MEM_BANK);
	ret += lsm6dsv_read_reg(ctx, LSM6DSV_MASTER_CONFIG,
	...
	ret = lsm6dsv_write_reg(ctx, LSM6DSV_MASTER_CONFIG,
				(uint8_t *)&master_config, 1);
			     ...
			     lsm6dsv_sh_cfg_write_t *val)
{
	...
	ret = lsm6dsv_write_reg(ctx, LSM6DSV_SLV0_SUBADD, &(val->slv0_subadd),
				1);
	if (ret != 0) {
		goto exit;
	}

	ret = lsm6dsv_write_reg(ctx, LSM6DSV_DATAWRITE_SLV0, &(val->slv0_data),
				1);
...
int32_t lsm6dsv_sigmot_mode_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_EMB_FUNC_EN_A,
...
int32_t lsm6dsv_stpcnt_rst_step_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	ret = lsm6dsv_write_reg(ctx, LSM6DSV_EMB_FUNC_SRC,
				(uint8_t *)&emb_func_src, 1);
...
int32_t lsm6dsv_stpcnt_debounce_set(const stmdev_ctx_t *ctx, uint8_t val)
{
	...
	ret = lsm6dsv_ln_pg_read(
		ctx, LSM6DSV_EMB_ADV_PG_1 + LSM6DSV_PEDO_DEB_STEPS_CONF,
...
int32_t lsm6dsv_stpcnt_period_set(const stmdev_ctx_t *ctx, uint16_t val)
{
	...
	buff[0] = (uint8_t)(val - (buff[1] * 256U));
	ret = lsm6dsv_ln_pg_write(
		ctx, LSM6DSV_EMB_ADV_PG_1 + LSM6DSV_PEDO_SC_DELTAT_L,
		(uint8_t *)&buff[0], 2);
...
int32_t lsm6dsv_stpcnt_period_get(const stmdev_ctx_t *ctx, uint16_t *val)
{
	...
	ret = lsm6dsv_ln_pg_read(
		ctx, LSM6DSV_EMB_ADV_PG_1 + LSM6DSV_PEDO_SC_DELTAT_L, &buff[0],
		2);
	if (ret != 0) {
		return ret;
	}
				   ...
				   lsm6dsv_sflp_data_rate_t val)
{
	...
	ret += lsm6dsv_write_reg(ctx, LSM6DSV_SFLP_ODR, (uint8_t *)&sflp_odr,
				 1);
				   ...
				   lsm6dsv_sflp_data_rate_t *val)
{
	...
	ret = lsm6dsv_mem_bank_set(ctx, LSM6DSV_EMBED_FUNC_MEM_BANK);
	if (ret == 0) {
		ret += lsm6dsv_read_reg(ctx, LSM6DSV_SFLP_ODR,
					(uint8_t *)&sflp_odr, 1);
	}
	ret += lsm6dsv_mem_bank_set(ctx, LSM6DSV_MAIN_MEM_BANK);
	if (ret != 0) {
				   ...
				   lsm6dsv_tap_thresholds_t val)
{
	...
	ret = lsm6dsv_write_reg(ctx, LSM6DSV_TAP_THS_6D, (uint8_t *)&tap_ths_6d,
				1);
	ret += lsm6dsv_write_reg(ctx, LSM6DSV_TAP_CFG2, (uint8_t *)&tap_cfg2,
				 1);
	ret += lsm6dsv_write_reg(ctx, LSM6DSV_TAP_CFG1, (uint8_t *)&tap_cfg1,
				 1);
				      ...
				      lsm6dsv_tap_axis_priority_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_TAP_CFG1,
					(uint8_t *)&tap_cfg1, 1);
	}
				     ...
				     lsm6dsv_tap_time_windows_t val)
{
	...
	if (ret == 0) {
		...
		ret = lsm6dsv_write_reg(ctx, LSM6DSV_TAP_DUR,
					(uint8_t *)&tap_dur, 1);
	}
...
/**
  ...
  */
int32_t
lsm6dsv_act_from_sleep_to_act_dur_set(const stmdev_ctx_t *ctx,
				      lsm6dsv_act_from_sleep_to_act_dur_t val)
{
	lsm6dsv_inactivity_dur_t inactivity_dur;
	int32_t ret;

	ret = lsm6dsv_read_reg(ctx, LSM6DSV_INACTIVITY_DUR,
...
/**
  ...
  */
int32_t
lsm6dsv_act_from_sleep_to_act_dur_get(const stmdev_ctx_t *ctx,
				      lsm6dsv_act_from_sleep_to_act_dur_t *val)
{
	lsm6dsv_inactivity_dur_t inactivity_dur;
	int32_t ret;
				   ...
				   lsm6dsv_act_thresholds_t *val)
{
	...
	ret = lsm6dsv_read_reg(ctx, LSM6DSV_INACTIVITY_DUR,
			       (uint8_t *)&inactivity_dur, 1);
	ret += lsm6dsv_read_reg(ctx, LSM6DSV_INACTIVITY_THS,
