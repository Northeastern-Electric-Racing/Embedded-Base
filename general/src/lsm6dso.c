...
HAL_StatusTypeDef lsm6dso_read_gyro(lsm6dso_t *imu) {
  ...
  status =
      lsm6dso_read_mult_reg(imu, gyro_x_raw.buf, LSM6DSO_REG_GYRO_X_AXIS_L, 2);
  if (status != HAL_OK)
    return status;

  status =
      lsm6dso_read_mult_reg(imu, gyro_y_raw.buf, LSM6DSO_REG_GYRO_Y_AXIS_L, 2);
  if (status != HAL_OK)
    return status;

  status =
      lsm6dso_read_mult_reg(imu, gyro_z_raw.buf, LSM6DSO_REG_GYRO_Z_AXIS_L, 2);
  if (status != HAL_OK)
    return status;
