...
static int32_t
LSM6DSO_ACC_SetOutputDataRate_When_Enabled(LSM6DSO_Object_t *pObj, float_t Odr);
static int32_t
LSM6DSO_ACC_SetOutputDataRate_When_Disabled(LSM6DSO_Object_t *pObj,
					    float_t Odr);
static int32_t
LSM6DSO_GYRO_SetOutputDataRate_When_Enabled(LSM6DSO_Object_t *pObj,
					    float_t Odr);
static int32_t
LSM6DSO_GYRO_SetOutputDataRate_When_Disabled(LSM6DSO_Object_t *pObj,
					     float_t Odr);
...
int32_t LSM6DSO_RegisterBusIO(LSM6DSO_Object_t *pObj, LSM6DSO_IO_t *pIO)
{
	...
	if (pObj == NULL) {
		...
		if (pObj->IO.Init == NULL) {
			...
			if (pObj->IO.BusType ==
			    LSM6DSO_SPI_3WIRES_BUS) /* SPI 3-Wires */
			{
				...
				if (pObj->is_initialized == 0U) {
					...
					if (LSM6DSO_Write_Reg(
						    pObj, LSM6DSO_CTRL3_C,
						    data) != LSM6DSO_OK) {
						ret = LSM6DSO_ERROR;
					}
				}
...
{
	/* Disable I3C */
	if (lsm6dso_i3c_disable_set(&(pObj->Ctx), LSM6DSO_I3C_DISABLE) !=
	    LSM6DSO_OK) {
  ...
  access with a serial interface. */
	if (lsm6dso_auto_increment_set(&(pObj->Ctx), PROPERTY_ENABLE) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}

	/* Enable BDU */
	if (lsm6dso_block_data_update_set(&(pObj->Ctx), PROPERTY_ENABLE) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}

	/* FIFO mode selection */
	if (lsm6dso_fifo_mode_set(&(pObj->Ctx), LSM6DSO_BYPASS_MODE) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
	...
	if (lsm6dso_xl_data_rate_set(&(pObj->Ctx), LSM6DSO_XL_ODR_OFF) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
	...
	/* Output data rate selection - power down. */
	if (lsm6dso_gy_data_rate_set(&(pObj->Ctx), LSM6DSO_GY_ODR_OFF) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}

	/* Full scale selection. */
	if (lsm6dso_gy_full_scale_set(&(pObj->Ctx), LSM6DSO_2000dps) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_GetCapabilities(LSM6DSO_Object_t *pObj,
				LSM6DSO_Capabilities_t *Capabilities)
{
...
int32_t LSM6DSO_ACC_Enable(LSM6DSO_Object_t *pObj)
{
	...
	if (lsm6dso_xl_data_rate_set(&(pObj->Ctx), pObj->acc_odr) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
int32_t LSM6DSO_ACC_Disable(LSM6DSO_Object_t *pObj)
{
	...
	if (lsm6dso_xl_data_rate_get(&(pObj->Ctx), &pObj->acc_odr) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
int32_t LSM6DSO_ACC_GetSensitivity(LSM6DSO_Object_t *pObj, float_t *Sensitivity)
{
	...
	if (lsm6dso_xl_full_scale_get(&(pObj->Ctx), &full_scale) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
int32_t LSM6DSO_ACC_GetOutputDataRate(LSM6DSO_Object_t *pObj, float_t *Odr)
{
	...
	/* Get current output data rate. */
	if (lsm6dso_xl_data_rate_get(&(pObj->Ctx), &odr_low_level) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
{
	return LSM6DSO_ACC_SetOutputDataRate_With_Mode(
		pObj, Odr, LSM6DSO_ACC_HIGH_PERFORMANCE_MODE);
}
...
/**
  ...
  */
int32_t
LSM6DSO_ACC_SetOutputDataRate_With_Mode(LSM6DSO_Object_t *pObj, float_t Odr,
					LSM6DSO_ACC_Operating_Mode_t Mode)
{
	...
	case LSM6DSO_ACC_HIGH_PERFORMANCE_MODE: {
		...
		if (lsm6dso_read_reg(&(pObj->Ctx), LSM6DSO_CTRL5_C,
				     (uint8_t *)&val1, 1) != LSM6DSO_OK) {
			return LSM6DSO_ERROR;
		}
		...
		if (val1.xl_ulp_en != 0U) {
			...
			if (pObj->acc_is_enabled == 1U) {
				if (lsm6dso_xl_data_rate_set(
					    &(pObj->Ctx), LSM6DSO_XL_ODR_OFF) !=
				    LSM6DSO_OK) {
					return LSM6DSO_ERROR;
				}
			}
			...
			val1.xl_ulp_en = 0;
			if (lsm6dso_write_reg(&(pObj->Ctx), LSM6DSO_CTRL5_C,
					      (uint8_t *)&val1,
					      1) != LSM6DSO_OK) {
				return LSM6DSO_ERROR;
			}
		...
		if (lsm6dso_read_reg(&(pObj->Ctx), LSM6DSO_CTRL6_C,
				     (uint8_t *)&val2, 1) != LSM6DSO_OK) {
			return LSM6DSO_ERROR;
		}

		if (val2.xl_hm_mode != 0U) {
			val2.xl_hm_mode = 0U;
			if (lsm6dso_write_reg(&(pObj->Ctx), LSM6DSO_CTRL6_C,
					      (uint8_t *)&val2,
					      1) != LSM6DSO_OK) {
				return LSM6DSO_ERROR;
			}
...
int32_t LSM6DSO_ACC_GetFullScale(LSM6DSO_Object_t *pObj, int32_t *FullScale)
{
	...
	/* Read actual full scale selection from sensor. */
	if (lsm6dso_xl_full_scale_get(&(pObj->Ctx), &fs_low_level) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
int32_t LSM6DSO_ACC_GetAxesRaw(LSM6DSO_Object_t *pObj, LSM6DSO_AxesRaw_t *Value)
{
	...
	if (lsm6dso_acceleration_raw_get(&(pObj->Ctx), data_raw.i16bit) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_GetAxes(LSM6DSO_Object_t *pObj,
			    LSM6DSO_Axes_t *Acceleration)
{
	...
	/* Calculate the data. */
	Acceleration->x =
		(int32_t)((float_t)((float_t)data_raw.i16bit[0] * sensitivity));
	Acceleration->y =
		(int32_t)((float_t)((float_t)data_raw.i16bit[1] * sensitivity));
	Acceleration->z =
		(int32_t)((float_t)((float_t)data_raw.i16bit[2] * sensitivity));
...
int32_t LSM6DSO_GYRO_Enable(LSM6DSO_Object_t *pObj)
{
	...
	/* Output data rate selection. */
	if (lsm6dso_gy_data_rate_set(&(pObj->Ctx), pObj->gyro_odr) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
int32_t LSM6DSO_GYRO_Disable(LSM6DSO_Object_t *pObj)
{
	...
	/* Get current output data rate. */
	if (lsm6dso_gy_data_rate_get(&(pObj->Ctx), &pObj->gyro_odr) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_GYRO_GetSensitivity(LSM6DSO_Object_t *pObj,
				    float_t *Sensitivity)
{
	int32_t ret = LSM6DSO_OK;
	...
	/* Read actual full scale selection from sensor. */
	if (lsm6dso_gy_full_scale_get(&(pObj->Ctx), &full_scale) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
int32_t LSM6DSO_GYRO_GetOutputDataRate(LSM6DSO_Object_t *pObj, float_t *Odr)
{
	...
	/* Get current output data rate. */
	if (lsm6dso_gy_data_rate_get(&(pObj->Ctx), &odr_low_level) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
{
	return LSM6DSO_GYRO_SetOutputDataRate_With_Mode(
		pObj, Odr, LSM6DSO_GYRO_HIGH_PERFORMANCE_MODE);
}
...
/**
  ...
  */
int32_t
LSM6DSO_GYRO_SetOutputDataRate_With_Mode(LSM6DSO_Object_t *pObj, float_t Odr,
					 LSM6DSO_GYRO_Operating_Mode_t Mode)
{
	...
	case LSM6DSO_GYRO_HIGH_PERFORMANCE_MODE: {
		...
		if (lsm6dso_read_reg(&(pObj->Ctx), LSM6DSO_CTRL7_G,
				     (uint8_t *)&val1, 1) != LSM6DSO_OK) {
			return LSM6DSO_ERROR;
		}
		...
		if (val1.g_hm_mode != 0U) {
			val1.g_hm_mode = 0U;
			if (lsm6dso_write_reg(&(pObj->Ctx), LSM6DSO_CTRL7_G,
					      (uint8_t *)&val1,
					      1) != LSM6DSO_OK) {
				return LSM6DSO_ERROR;
			}
	...
	if (pObj->gyro_is_enabled == 1U) {
		...
		ret = LSM6DSO_GYRO_SetOutputDataRate_When_Disabled(pObj,
								   newOdr);
	}
...
int32_t LSM6DSO_GYRO_GetFullScale(LSM6DSO_Object_t *pObj, int32_t *FullScale)
{
	...
	/* Read actual full scale selection from sensor. */
	if (lsm6dso_gy_full_scale_get(&(pObj->Ctx), &fs_low_level) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_GYRO_GetAxesRaw(LSM6DSO_Object_t *pObj,
				LSM6DSO_AxesRaw_t *Value)
{
	lsm6dso_axis3bit16_t data_raw;

	/* Read raw data values. */
	if (lsm6dso_angular_rate_raw_get(&(pObj->Ctx), data_raw.i16bit) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_GYRO_GetAxes(LSM6DSO_Object_t *pObj,
...
int32_t LSM6DSO_Set_Interrupt_Latch(LSM6DSO_Object_t *pObj, uint8_t Status)
{
	...
	if (lsm6dso_int_notification_set(&(pObj->Ctx), newStatus) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Enable_Free_Fall_Detection(LSM6DSO_Object_t *pObj,
					       LSM6DSO_SensorIntPin_t IntPin)
{
	...
	/* FF_THS setting */
	if (lsm6dso_ff_threshold_set(&(pObj->Ctx), LSM6DSO_FF_TSH_312mg) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
	...
	case LSM6DSO_INT1_PIN:
		if (lsm6dso_pin_int1_route_get(&(pObj->Ctx), &val1) !=
		    LSM6DSO_OK) {
			return LSM6DSO_ERROR;
		}
		...
		if (lsm6dso_pin_int1_route_set(&(pObj->Ctx), val1) !=
		    LSM6DSO_OK) {
			return LSM6DSO_ERROR;
		}
		break;
	...
	case LSM6DSO_INT2_PIN:
		if (lsm6dso_pin_int2_route_get(&(pObj->Ctx), NULL, &val2) !=
		    LSM6DSO_OK) {
			return LSM6DSO_ERROR;
		}
		...
		if (lsm6dso_pin_int2_route_set(&(pObj->Ctx), NULL, val2) !=
		    LSM6DSO_OK) {
			return LSM6DSO_ERROR;
		}
		break;
...
int32_t LSM6DSO_ACC_Disable_Free_Fall_Detection(LSM6DSO_Object_t *pObj)
{
	...
	/* FF_THS setting */
	if (lsm6dso_ff_threshold_set(&(pObj->Ctx), LSM6DSO_FF_TSH_156mg) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Set_Free_Fall_Threshold(LSM6DSO_Object_t *pObj,
					    uint8_t Threshold)
{
	int32_t ret = LSM6DSO_OK;
	...
	if (lsm6dso_ff_threshold_set(&(pObj->Ctx), newThreshold) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Set_Free_Fall_Duration(LSM6DSO_Object_t *pObj,
					   uint8_t Duration)
{
...
int32_t LSM6DSO_ACC_Enable_Pedometer(LSM6DSO_Object_t *pObj)
{
	...
	if (lsm6dso_pedo_sens_set(&(pObj->Ctx), LSM6DSO_PEDO_BASE_MODE) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
{
	if (lsm6dso_number_of_steps_get(&(pObj->Ctx), StepCount) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Enable_Tilt_Detection(LSM6DSO_Object_t *pObj,
					  LSM6DSO_SensorIntPin_t IntPin)
{
	int32_t ret = LSM6DSO_OK;
	lsm6dso_pin_int1_route_t val1;
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Enable_Wake_Up_Detection(LSM6DSO_Object_t *pObj,
					     LSM6DSO_SensorIntPin_t IntPin)
{
	int32_t ret = LSM6DSO_OK;
	lsm6dso_pin_int1_route_t val1;
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Set_Wake_Up_Threshold(LSM6DSO_Object_t *pObj,
					  uint8_t Threshold)
{
	/* Set wake up threshold. */
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Set_Wake_Up_Duration(LSM6DSO_Object_t *pObj,
					 uint8_t Duration)
{
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Enable_Single_Tap_Detection(LSM6DSO_Object_t *pObj,
						LSM6DSO_SensorIntPin_t IntPin)
{
	int32_t ret = LSM6DSO_OK;
	lsm6dso_pin_int1_route_t val1;
	...
	if (lsm6dso_tap_detection_on_x_set(&(pObj->Ctx), PROPERTY_ENABLE) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}

	/* Enable Y direction in tap recognition. */
	if (lsm6dso_tap_detection_on_y_set(&(pObj->Ctx), PROPERTY_ENABLE) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}

	/* Enable Z direction in tap recognition. */
	if (lsm6dso_tap_detection_on_z_set(&(pObj->Ctx), PROPERTY_ENABLE) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
int32_t LSM6DSO_ACC_Disable_Single_Tap_Detection(LSM6DSO_Object_t *pObj)
{
	...
	if (lsm6dso_tap_detection_on_z_set(&(pObj->Ctx), PROPERTY_DISABLE) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}

	/* Disable Y direction in tap recognition. */
	if (lsm6dso_tap_detection_on_y_set(&(pObj->Ctx), PROPERTY_DISABLE) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}

	/* Disable X direction in tap recognition. */
	if (lsm6dso_tap_detection_on_x_set(&(pObj->Ctx), PROPERTY_DISABLE) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Enable_Double_Tap_Detection(LSM6DSO_Object_t *pObj,
						LSM6DSO_SensorIntPin_t IntPin)
{
	int32_t ret = LSM6DSO_OK;
	lsm6dso_pin_int1_route_t val1;
	...
	if (lsm6dso_tap_mode_set(&(pObj->Ctx), LSM6DSO_BOTH_SINGLE_DOUBLE) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
int32_t LSM6DSO_ACC_Disable_Double_Tap_Detection(LSM6DSO_Object_t *pObj)
{
	...
	if (lsm6dso_tap_mode_set(&(pObj->Ctx), LSM6DSO_ONLY_SINGLE) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
{
	/* Set tap threshold. */
	if (lsm6dso_tap_threshold_x_set(&(pObj->Ctx), Threshold) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Enable_6D_Orientation(LSM6DSO_Object_t *pObj,
					  LSM6DSO_SensorIntPin_t IntPin)
{
	int32_t ret = LSM6DSO_OK;
	lsm6dso_pin_int1_route_t val1;
	...
	if (lsm6dso_6d_threshold_set(&(pObj->Ctx), LSM6DSO_DEG_60) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
int32_t LSM6DSO_ACC_Disable_6D_Orientation(LSM6DSO_Object_t *pObj)
{
	...
	if (lsm6dso_6d_threshold_set(&(pObj->Ctx), LSM6DSO_DEG_80) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Set_6D_Orientation_Threshold(LSM6DSO_Object_t *pObj,
						 uint8_t Threshold)
{
	int32_t ret = LSM6DSO_OK;
	...
	if (lsm6dso_6d_threshold_set(&(pObj->Ctx), newThreshold) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
int32_t LSM6DSO_ACC_Get_6D_Orientation_XL(LSM6DSO_Object_t *pObj, uint8_t *XLow)
{
	...
	if (lsm6dso_read_reg(&(pObj->Ctx), LSM6DSO_D6D_SRC, (uint8_t *)&data,
			     1) != LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Get_6D_Orientation_XH(LSM6DSO_Object_t *pObj,
					  uint8_t *XHigh)
{
	lsm6dso_d6d_src_t data;
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Get_6D_Orientation_YH(LSM6DSO_Object_t *pObj,
					  uint8_t *YHigh)
{
	lsm6dso_d6d_src_t data;
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Get_6D_Orientation_ZH(LSM6DSO_Object_t *pObj,
					  uint8_t *ZHigh)
{
	lsm6dso_d6d_src_t data;
...
{
	if (lsm6dso_xl_flag_data_ready_get(&(pObj->Ctx), Status) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Get_Event_Status(LSM6DSO_Object_t *pObj,
				     LSM6DSO_Event_Status_t *Status)
{
	...
	if (lsm6dso_read_reg(&(pObj->Ctx), LSM6DSO_WAKE_UP_SRC,
			     (uint8_t *)&wake_up_src, 1) != LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}

	if (lsm6dso_read_reg(&(pObj->Ctx), LSM6DSO_TAP_SRC, (uint8_t *)&tap_src,
			     1) != LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}

	if (lsm6dso_read_reg(&(pObj->Ctx), LSM6DSO_D6D_SRC, (uint8_t *)&d6d_src,
			     1) != LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}

	if (lsm6dso_mem_bank_set(&(pObj->Ctx), LSM6DSO_EMBEDDED_FUNC_BANK) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}

	if (lsm6dso_read_reg(&(pObj->Ctx), LSM6DSO_EMB_FUNC_SRC,
			     (uint8_t *)&func_src, 1) != LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}

	if (lsm6dso_read_reg(&(pObj->Ctx), LSM6DSO_EMB_FUNC_INT1,
			     (uint8_t *)&int1_ctrl, 1) != LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}

	if (lsm6dso_read_reg(&(pObj->Ctx), LSM6DSO_EMB_FUNC_INT2,
			     (uint8_t *)&int2_ctrl, 1) != LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
	...
	if (lsm6dso_read_reg(&(pObj->Ctx), LSM6DSO_MD1_CFG, (uint8_t *)&md1_cfg,
			     1) != LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}

	if (lsm6dso_read_reg(&(pObj->Ctx), LSM6DSO_MD2_CFG, (uint8_t *)&md2_cfg,
			     1) != LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}

	if (lsm6dso_tilt_flag_data_ready_get(&(pObj->Ctx), &tilt_ia) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
	...
	if ((md1_cfg.int1_single_tap == 1U) ||
	    (md2_cfg.int2_single_tap == 1U)) {
		if (tap_src.single_tap == 1U) {
			Status->TapStatus = 1;
		}
	}

	if ((md1_cfg.int1_double_tap == 1U) ||
	    (md2_cfg.int2_double_tap == 1U)) {
...
{
	if (lsm6dso_gy_flag_data_ready_get(&(pObj->Ctx), Status) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_FIFO_Get_Num_Samples(LSM6DSO_Object_t *pObj,
				     uint16_t *NumSamples)
{
	if (lsm6dso_fifo_data_level_get(&(pObj->Ctx), NumSamples) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
int32_t LSM6DSO_FIFO_Get_Full_Status(LSM6DSO_Object_t *pObj, uint8_t *Status)
{
	...
	if (lsm6dso_read_reg(&(pObj->Ctx), LSM6DSO_FIFO_STATUS2, &reg.byte,
			     1) != LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
int32_t LSM6DSO_FIFO_Set_INT1_FIFO_Full(LSM6DSO_Object_t *pObj, uint8_t Status)
{
	...
	if (lsm6dso_read_reg(&(pObj->Ctx), LSM6DSO_INT1_CTRL, &reg.byte, 1) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
	...
	if (lsm6dso_write_reg(&(pObj->Ctx), LSM6DSO_INT1_CTRL, &reg.byte, 1) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_FIFO_Set_Watermark_Level(LSM6DSO_Object_t *pObj,
					 uint16_t Watermark)
{
...
int32_t LSM6DSO_FIFO_Get_Tag(LSM6DSO_Object_t *pObj, uint8_t *Tag)
{
	...
	if (lsm6dso_fifo_sensor_tag_get(&(pObj->Ctx), &tag_local) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
{
	if (lsm6dso_read_reg(&(pObj->Ctx), LSM6DSO_FIFO_DATA_OUT_X_L, Data,
			     6) != LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_FIFO_ACC_Get_Axes(LSM6DSO_Object_t *pObj,
				  LSM6DSO_Axes_t *Acceleration)
{
	lsm6dso_axis3bit16_t data_raw;
	float_t sensitivity = 0.0f;
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_FIFO_GYRO_Get_Axes(LSM6DSO_Object_t *pObj,
...
int32_t LSM6DSO_ACC_Enable_DRDY_On_INT1(LSM6DSO_Object_t *pObj)
{
	...
	if (lsm6dso_pin_int1_route_get(&(pObj->Ctx), &pin_int1_route) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
	pin_int1_route.drdy_xl = 1;
	pin_int1_route.drdy_g = 0;
	if (lsm6dso_pin_int1_route_set(&(pObj->Ctx), pin_int1_route) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
int32_t LSM6DSO_ACC_Set_Power_Mode(LSM6DSO_Object_t *pObj, uint8_t PowerMode)
{
	...
	if (lsm6dso_xl_power_mode_set(&(pObj->Ctx), newPowerMode) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Set_Filter_Mode(LSM6DSO_Object_t *pObj,
				    uint8_t LowHighPassFlag, uint8_t FilterMode)
{
	int32_t ret = LSM6DSO_OK;
	...
	if (LowHighPassFlag == 0U) {
		...
		if (lsm6dso_xl_hp_path_on_out_set(
			    &(pObj->Ctx), newFilterMode) != LSM6DSO_OK) {
			return LSM6DSO_ERROR;
		}
	} else {
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_ACC_Enable_Inactivity_Detection(LSM6DSO_Object_t *pObj,
						lsm6dso_inact_en_t InactMode,
						LSM6DSO_SensorIntPin_t IntPin)
{
	lsm6dso_pin_int1_route_t val1;
	lsm6dso_pin_int2_route_t val2;
	...
	case LSM6DSO_XL_AND_GY_NOT_AFFECTED: {
		if (lsm6dso_act_mode_set(&(pObj->Ctx),
					 LSM6DSO_XL_AND_GY_NOT_AFFECTED) !=
		    LSM6DSO_OK) {
			return LSM6DSO_ERROR;
		}
		break;
	...
	case LSM6DSO_XL_12Hz5_GY_NOT_AFFECTED: {
		if (lsm6dso_act_mode_set(&(pObj->Ctx),
					 LSM6DSO_XL_12Hz5_GY_NOT_AFFECTED) !=
		    LSM6DSO_OK) {
			return LSM6DSO_ERROR;
		}
		break;
...
int32_t LSM6DSO_GYRO_Enable_DRDY_On_INT2(LSM6DSO_Object_t *pObj)
{
	...
	if (lsm6dso_pin_int2_route_get(&(pObj->Ctx), NULL, &pin_int2_route) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
	pin_int2_route.drdy_xl = 0;
	pin_int2_route.drdy_g = 1;
	if (lsm6dso_pin_int2_route_set(&(pObj->Ctx), NULL, pin_int2_route) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
int32_t LSM6DSO_GYRO_Set_Power_Mode(LSM6DSO_Object_t *pObj, uint8_t PowerMode)
{
	...
	if (lsm6dso_gy_power_mode_set(&(pObj->Ctx), newPowerMode) !=
	    LSM6DSO_OK) {
		return LSM6DSO_ERROR;
	}
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t LSM6DSO_GYRO_Set_Filter_Mode(LSM6DSO_Object_t *pObj,
				     ...
				     uint8_t FilterMode)
{
	...
	if (LowHighPassFlag == 0U) {
		...
		if (lsm6dso_gy_lp1_bandwidth_set(&(pObj->Ctx), newFilterMode) !=
		    LSM6DSO_OK) {
			return LSM6DSO_ERROR;
		}
	} else {
...
/**
  ...
  */
static int32_t
LSM6DSO_ACC_SetOutputDataRate_When_Disabled(LSM6DSO_Object_t *pObj, float_t Odr)
{
...
/**
  ...
  */
static int32_t
LSM6DSO_GYRO_SetOutputDataRate_When_Enabled(LSM6DSO_Object_t *pObj, float_t Odr)
{
