...
#define ETH_MAX_PACKETS \
	10 /* Maximum number of packets we wanna handle simultaneously */
...
typedef void (*DriverFunction)(
	NX_IP_DRIVER
		*); /* User-supplied network driver used to send and receive IP packets. */
typedef void (*OnRecieve)(
	ethernet_message_t
		message); /* User-supplied function that will be called whenever an ethernet message is recieved. */
