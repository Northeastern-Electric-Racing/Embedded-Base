...
static void _receive_message(NX_UDP_SOCKET *socket)
{
	...
	if (status == NX_SUCCESS) {
		...
		if (bytes_copied < sizeof(ethernet_message_t)) {
			DEBUG_PRINTLN(
				"WARNING: Received ethernet message was smaller than expected (only received %lu of %u expected bytes).",
		...
		if (status == NX_SUCCESS) {
			DEBUG_PRINTLN(
				"Received ethernet message! (Sender ID: %d, Message ID: %d).",
...
uint8_t ethernet_init(ethernet_node_t node_id, DriverFunction driver,
		      OnRecieve on_recieve)
{
	uint8_t status;
	...
	/* Store node id and user-set functions. */
	...
	if (status != NX_SUCCESS) {
		DEBUG_PRINTLN(
			"ERROR: Failed to create packet pool (Status: %d/%s).",
			status, nx_status_toString(status));
		return status;
	}
	...
	status = nx_ip_create(
		...
		IP_ADDRESS(
			5, 5, 5,
			device.node_id), // Dummy unicast IP (we shouldn't have to use this if we're just using multicast for everything)
	...
	if (status != NX_SUCCESS) {
		DEBUG_PRINTLN(
			"ERROR: Failed to create IP instance (Status: %d/%s).",
			status, nx_status_toString(status));
		return status;
	}

	/* Enable ARP */
	status = nx_arp_enable(&device.ip, // IP instance
			       device.arp_cache_memory, // Memory for ARP cache
			       _ARP_CACHE_SIZE // Size of ARP cache
	);
	if (status != NX_SUCCESS) {
		DEBUG_PRINTLN("ERROR: Failed to enable ARP (Status: %d/%s).",
			      status, nx_status_toString(status));
		return status;
	}
	...
	if (status != NX_SUCCESS) {
		DEBUG_PRINTLN("ERROR: Failed to enable UDP (Status: %d/%s).",
			      status, nx_status_toString(status));
		return status;
	}
	...
	if (status != NX_SUCCESS) {
		DEBUG_PRINTLN("ERROR: Failed to enable igmp (Status: %d/%s).",
			      status, nx_status_toString(status));
		return status;
	}
    ...
    */
	for (int i = (1 << 0); i < (1 << 8); i++) {
		if (i & device.node_id) {
			...
			if (status != NX_SUCCESS) {
				DEBUG_PRINTLN(
					"ERROR: Failed to join multicast group (Status: %d/%s, Address: %lu).",
					status, nx_status_toString(status),
					address);
			}
		}
	...
	status = nx_udp_socket_create(&device.ip, // IP instance
	...
	if (status != NX_SUCCESS) {
		DEBUG_PRINTLN(
			"ERROR: Failed to create UDP socket (Status: %d/%s).",
			status, nx_status_toString(status));
		return status;
	}

	/* Bind socket to broadcast port */
	status = nx_udp_socket_bind(&device.socket, // Socket to bind
				    ETH_UDP_PORT, // Port
				    TX_WAIT_FOREVER // Wait forever
	);
	if (status != NX_SUCCESS) {
		DEBUG_PRINTLN(
			"ERROR: Failed to bind UDP socket (Status: %d/%s).",
			status, nx_status_toString(status));
	...
	if (status != NX_SUCCESS) {
		DEBUG_PRINTLN(
			"ERROR: Failed to set recieve callback (Status: %d/%s).",
			status, nx_status_toString(status));
...
ethernet_message_t ethernet_create_message(uint8_t message_id,
					   ethernet_node_t recipient_id,
					   uint8_t *data, uint8_t data_length)
{
	...
	if (data_length > ETH_MESSAGE_SIZE) {
		DEBUG_PRINTLN(
			"ERROR: Data length exceeds maximum (message_id: %d).",
			message_id);
...
uint8_t ethernet_send_message(ethernet_message_t *message)
{
	...
	if (!device.is_initialized) {
		DEBUG_PRINTLN(
			"ERROR: Ethernet device is not initialized, so ethernet_send_message() will not work.");
		return U_ERROR;
	}
	...
	if (message->data_length > ETH_MESSAGE_SIZE) {
		DEBUG_PRINTLN(
			"ERROR: Data length exceeds maximum (Message ID: %d).",
			message->message_id);
		return U_ERROR;
	}
	...
	if (status != NX_SUCCESS) {
		DEBUG_PRINTLN(
			"ERROR: Failed to allocate packet (Status: %d/%s, Message ID: %d).",
			status, nx_status_toString(status),
			message->message_id);
		return U_ERROR;
	}
	...
	if (status != NX_SUCCESS) {
		DEBUG_PRINTLN(
			"ERROR: Failed to append data to packet (Status: %d/%s, Message ID: %d).",
			status, nx_status_toString(status),
			message->message_id);
		nx_packet_release(packet);
		return U_ERROR;
	...
	/* Send message */
	status = nx_udp_socket_send(&device.socket, packet,
				    ETH_IP(message->recipient_id),
				    ETH_UDP_PORT);
	if (status != NX_SUCCESS) {
		DEBUG_PRINTLN(
			"ERROR: Failed to send packet (Status: %d/%s, Message ID: %d).",
			status, nx_status_toString(status),
			message->message_id);
		nx_packet_release(packet);
		return U_ERROR;
	...
	DEBUG_PRINTLN(
		"Sent ethernet message (Recipient ID: %d, Message ID: %d).",
