	...
	switch (operation) {
		case NX_PTP_CLIENT_CLOCK_INIT:
			...
			if (1000000000 % pll1.PLL1_Q_Frequency != 0) {
				...
				PRINTLN_ERROR("NOT PERFEECT DIVISOR PTP TIME WILL BE OFF -> CHECK PLL1Q!");
			}
			uint32_t period_ns = 1000000000 / pll1.PLL1_Q_Frequency;
			MODIFY_REG(heth.Instance->MACSSIR, ETH_MACMACSSIR_SNSINC, period_ns << ETH_MACMACSSIR_SSINC_Pos);
		...
		case NX_PTP_CLIENT_CLOCK_ADJUST:
			...
			if (time_ptr->nanosecond >= 0) {
				...
				uint32_t complement = 1000000000 - abs(time_ptr->nanosecond);
