...
namespace Antmicro.Renode.Peripherals.I2C
{
    ...
    public sealed class STM32F4_I2C_NER : SimpleContainer<II2CPeripheral>, IDoubleWordPeripheral, IBytePeripheral, IKnownSize
    {
        ...
        {
            // this.NoisyLog("ReadDouble!");
            return registers.Read(offset);
        }

        public void WriteDoubleWord(long offset, uint value)
        {
            this.NoisyLog("WriteDouble!");
        ...
        private void DataWrite(uint oldValue, uint newValue)
        {
            ...
            switch (state)
            {
                case State.AwaitingAddress:
                    ...
                    {
                        this.WarningLog("Child not found:\t{0:X}", address);
                        state = State.Idle;
        ...
        {
            this.NoisyLog("PeripheralEnableWrite!");
            if (!newValue)
            {
