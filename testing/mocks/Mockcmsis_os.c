...
void osSignalWait_CMockExpectAndReturn(UNITY_LINE_TYPE cmock_line, int32_t signals, uint32_t millisec, os_InRegs osEvent cmock_to_return)
{
  ...
  memcpy((void*)(&cmock_call_instance->ReturnVal), (void*)(&cmock_to_return),
         sizeof(os_InRegs osEvent[sizeof(cmock_to_return) == sizeof(os_InRegs osEvent) ? 1 : -1])); /* add os_InRegs osEvent to :treat_as_array if this causes an error */
}
...
void CMockExpectParameters_osMailPut(CMOCK_osMailPut_CALL_INSTANCE* cmock_call_instance, osMailQId queue_id, const void* mail);
void CMockExpectParameters_osMailPut(CMOCK_osMailPut_CALL_INSTANCE* cmock_call_instance, osMailQId queue_id, const void* mail)
{
  memcpy((void*)(&cmock_call_instance->Expected_queue_id), (void*)(&queue_id),
         sizeof(osMailQId[sizeof(queue_id) == sizeof(osMailQId) ? 1 : -1])); /* add osMailQId to :treat_as_array if this causes an error */
