...
HAL_StatusTypeDef eeprom_read(uint16_t mem_address, uint8_t *data, uint16_t size)
{
    ...
    for(int bytes_read = 0; bytes_read < size; bytes_read += 32)
    {
        ...
        status = HAL_I2C_Mem_Read(i2c_handle, M24C32_I2C_ADDR, mem_address + bytes_read, 2, &data[bytes_read],
                                  read_size, 1000);
        if (status)
            return status;
    }
...
HAL_StatusTypeDef eeprom_delete(uint16_t mem_address, uint16_t size)
{
    ...
    for(int bytes_deleted = 0; bytes_deleted < size; bytes_deleted += 32)
    {
        ...
        status = HAL_I2C_Mem_Write(i2c_handle, M24C32_I2C_ADDR, mem_address + bytes_deleted, 2, &data[bytes_deleted], 
                                   delete_size, 1000);
        if (status)
            return status;
    }
