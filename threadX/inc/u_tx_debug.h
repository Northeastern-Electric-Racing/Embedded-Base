...
#define __FILENAME__                                           \
	(strrchr(__FILE__, '/') ? strrchr(__FILE__, '/') + 1 : \
				  __FILE__) /* Gets the name of the file. */
#define u_tx_debug /* Comment out to enable/disable debugging. */
#ifdef u_tx_debug
#define DEBUG_PRINTLN(message, ...)                               \
	printf("[%s/%s()] " message "\n", __FILENAME__, __func__, \
	       ##__VA_ARGS__) /* Prints an error message in the format: "[file_name.c/function()] {message}"*/
#else
#define DEBUG_PRINTLN( \
	message,       \
	...) /* If debugging is turned off, macro doesn't need to expand to anything. */
#endif
 ...
 */
#define CATCH_ERROR(function_call, success)                                        \
	do {                                                                       \
		int _function_status = (function_call);                            \
		if (_function_status != success) {                                 \
			DEBUG_PRINTLN(                                             \
				"CATCH_ERROR(): Function failed: %s (Status: %d)", \
				#function_call, _function_status);                 \
